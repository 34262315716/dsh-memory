...
export const Config = z.object({
  ...
  housekeeping: z.object({
    enabled: z.boolean().default(true),
    interval: z.number().min(10).max(1000).default(50),  // 每 N 轮自动巡检一次
...
export async function apply(ctx, config) {
  ...
  const turnTally = {}        // 管家巡检轮次计数（跨 session 共享）
  ctx.on('session/event', async (session, event) => {
    ...
    if (event.type === 'user/message') {
      ...
      // 阶段三⑥：管家低频自动巡检（只读观察：近重复/老化候选 → 日志；不擅改数据）
      const hk = getCfg().housekeeping ?? {}
      if (hk.enabled !== false) {
        const hkTurns = (turnTally.housekeepingTurns = (turnTally.housekeepingTurns ?? 0) + 1)
        if (hkTurns % (hk.interval ?? 50) === 0) {
          void (async () => {
            try {
