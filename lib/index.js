...
export async function apply(ctx, config) {
  ...
  // 运行日志 helper（v0.9.5）：logging.enabled 关闭时不写；日志失败不影响主流程
  ...
  // workspaceRegistry（v0.9.5）：EAC/web 会话无 cwd 时的 scope fallback
  let wsRegistry = null
...
function registerTools(ctx, store, getCfg) {
  ...
  safeRegister(defineTool({
    ...
    async execute(args, exec) {
      ...
      const toolScope = args.type === 'profile' ? 'global' : scopeOf(exec?.agent, wsRegistry)
  ...
  safeRegister(defineTool({
    ...
    async execute(args, exec) {
      const results = await store.search(args.query, {
        scope: [scopeOf(exec?.agent, wsRegistry), 'global'],
        limit: args.limit ?? 5,
        minScore: 0,
      })
  ...
  safeRegister(defineTool({
    ...
    execute(args, exec) {
      const rows = store.list({ scope: [scopeOf(exec?.agent, wsRegistry), 'global'], layer: args.layer, type: args.type, limit: args.limit ?? 20 })
      return {
        memories: rows.map((r) => ({
          id: r.id,
          layer: r.layer,
