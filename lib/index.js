...
export async function apply(ctx, config) {
  ...
  const hkState = { writtenSinceCheck: 0 }  // 管家：自上次巡检以来沉淀条数（lastAt 持久化在 meta 表）
  ctx.on('session/event', async (session, event) => {
    ...
    if (event.type === 'user/message') {
      ...
      // 阶段三⑥：管家自动巡检（写入量 + 时间双驱动，与对话轮数解耦——会话通常跑不满 50 轮）
      //   - 每沉淀 interval 条记忆触发一次（去重价值随库增长）
      //   - 距上次巡检超 maxIntervalHours 触发（时间兜底，跨重启持久化）
