        ...
        const disposeHealth = hostCtx.webServer.register({
          ...
          handler: async (request, response) => {
            try {
              ...
              const url = new URL(request.url ?? '/', 'http://localhost')
              const withLlm = url.searchParams.get('llm') === '1'
...
export async function buildHealthReport(ctx, store, cfg, { withLlm = false } = {}) {
  ...
  if (withLlm) {
    refiner.live = await probe(async () => {
      // 探针预算：默认 25 秒（自检要快），但不得比用户给真实提取的预算更宽松
      const cfgBudget = Number(ref.timeBudgetMs)
      const budgetMs = Math.min(25000, Number.isFinite(cfgBudget) && cfgBudget > 0 ? cfgBudget : 25000)
      ...
      throw new Error(`提取模型未输出正文。${fmt(a)}；${fmt(b)}；${fmt(c)}。（据此可判定：A 成功=修复生效；B 出现工具调用=tools 是元凶但空数组未生效；三者皆空=另有原因）`)
    })
  }
