...
export async function apply(ctx, config) {
  ...
  try {
    ...
    console.log(`[dsh-memory] embedder: ${embedder.name}（dim ${embedder.dim}）`)
    // 维度迁移后的后台重嵌入 + 主题聚类（不阻塞启动；迁移期 FTS/关键词路照常）
    void (async () => {
      try {
  ...
  // ---------- 阶段二：会话预热 seed（session-start 注入最近语义记忆） ----------
  ctx.on('agent/session-start', (payload) => {
    try {
      const cfg = getCfg()
      if (!cfg.features.preStepInject || !payload.agent) return
      const seeds = store.list({ layer: 'sm', limit: 3 })
      if (seeds.length === 0) return
      const lines = [
        '[记忆] 会话预热（最近的长期记忆，由 dsh-memory 注入）',
        ...seeds.map((s) => `- [${s.type}] ${truncate(s.content, 200)}`),
      ]
 ...
 * 实体级图（nodes/edges 表）在此投影为「记忆 = 节点」视图，供 Web GUI 渲染。
 */
function buildGraphSnapshot(store) {
  ...
  // 节点 → 关联记忆（node_memories 优先，fallback nodes.memory_id）
  const nodeMems = new Map()
  for (const r of store.db.prepare('SELECT node_id, memory_id FROM node_memories').all()) {
    if (!nodeMems.has(r.node_id)) nodeMems.set(r.node_id, [])
    nodeMems.get(r.node_id).push(r.memory_id)
  }
  const memOf = (nodeId) => {
    const via = nodeMems.get(nodeId)
    if (via && via.length > 0) return via[0]
    return store.db.prepare('SELECT memory_id FROM nodes WHERE id = ?').get(nodeId)?.memory_id
  }
  const edges = []
  const seen = new Set()
  ...
  for (const r of rows) {
    const a = memOf(r.from_node)
    const b = memOf(r.to_node)
    if (!a || !b || a === b) continue
    const key = r.type + '|' + a + '|' + b
    if (seen.has(key)) continue
    seen.add(key)
    edges.push({ from: a, to: b, type: r.type, weight: r.weight })
  }
  const themes = [...new Set(nodes.map((n) => n.theme).filter(Boolean))]
  return { stats: store.stats(), themes, nodes, edges }
}
