...
export function scopeOf(sessionOrAgent) {
...
export async function apply(ctx, config) {
  ...
  ctx.on('session/event', async (session, event) => {
    ...
    if (event.type === 'user/message') {
      ...
      const writeScope = (type) => (type === 'profile' ? 'global' : scopeOf(session))
...
// ---------- 阶段二：会话预热 seed（session-start 注入；画像优先 + 项目 scope 隔离） ----------
ctx.on('agent/session-start', (payload) => {
    try {
      ...
      const cur = scopeOf(payload.agent)
  ...
  ctx.on('agent/pre-step', async (payload, next) => {
    try {
      ...
      const hits = await store.search(text, {
        scope: [scopeOf(payload.agent), 'global'],
        limit: 6,
...
function registerTools(ctx, store, getCfg) {
  ...
  safeRegister(defineTool({
    ...
    async execute(args, exec) {
      ...
      const toolScope = args.type === 'profile' ? 'global' : scopeOf(exec?.agent)
  ...
  safeRegister(defineTool({
    ...
    async execute(args, exec) {
      const results = await store.search(args.query, {
        scope: [scopeOf(exec?.agent), 'global'],
        limit: args.limit ?? 5,
        minScore: 0,
      })
      return {
  ...
  safeRegister(defineTool({
    ...
    execute(args, exec) {
      const rows = store.list({ scope: [scopeOf(exec?.agent), 'global'], layer: args.layer, type: args.type, limit: args.limit ?? 20 })
      return {
        memories: rows.map((r) => ({
          id: r.id,
          layer: r.layer,
