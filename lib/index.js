...
ctx.on('agent/session-start', (payload) => {
    try {
      ...
      // 画像直接按 type 过滤取——不经过"最近 50 条"窗口（画像几乎永不更新，
      // updated_at 恒为创建时刻，走窗口会被近期活跃的普通记忆整体挤出）
      const profiles = store.list({ layer: 'sm', type: 'profile', limit: 3 })
      const others = store.list({ layer: 'sm', limit: 10 }).filter((m) => m.type !== 'profile').slice(0, 2)
...
function registerTools(ctx, store, getCfg) {
  ...
  safeRegister(defineTool({
    ...
    execute(args) {
      const rows = store.list({ scope: scope(), layer: args.layer, type: args.type, limit: args.limit ?? 20 })
      return {
        memories: rows.map((r) => ({
          id: r.id,
          layer: r.layer,
