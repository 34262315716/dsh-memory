...
export function apply(ctx, config) {
  ...
  ctx.on('session/event', (session, event) => {
    ...
    if (event.type === 'user/message') {
      ...
      if (config.features.valueGate) {
        if (!userPart && !assistantPart) return
        if (content.length < 20 && keywords.length === 0) return
      }
      upsertMemory(store, config.features, { layer: 'ep', scope, content, keywords })
    }
  })
  ...
  ctx.on('agent/pre-step', (payload, next) => {
    try {
      if (!config.features.preStepInject) return next()
      currentStep++
      ...
      if (last && currentStep - last.step < config.stepInterval) {
      ...
      // 4) token 预算：贪心装入
      const budget = config.injectMaxTokens
      let used = 0
      const picked = []
      for (const h of hits) {
      ...
      recentInjected.set(agentId, window.slice(-config.maxRecentPerAgent))
    } catch (err) {
  ...
  // ---------- 工具面 ----------
  if (config.features.manageTools) {
...
function registerTools(ctx, store, config) {
  ...
  ctx.tools.register(defineTool({
    ...
    execute(args) {
      const results = store.search(args.query, { scope, limit: args.limit ?? 5, minScore: 0 })
      return {
  ...
  ctx.tools.register(defineTool({
    ...
    execute(args) {
      const rows = store.list({ scope, layer: args.layer, limit: args.limit ?? 20 })
      return {
        memories: rows.map((r) => ({
          id: r.id,
          layer: r.layer,
