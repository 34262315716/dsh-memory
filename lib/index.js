...
function diag(...args) {
  try {
    appendFileSync(join(homedir(), '.dsh', 'dsh-memory.log'),
...
export function apply(ctx, config) {
  ...
  try {
    ...
    diag('settings 注册失败:', err.message, '\n', err.stack)
    throw err
  }
  const getCfg = () => settingsScope.get()
  if (!getCfg().enabled) {
    diag('enabled=false, 退出')
    return
  }
