          ...
          if (content.length < 40 || keywords.length < 3) {
            ...
            void (async () => {
              try {
                ...
                logStore('info', 'write.refined', { type: extracted.type, abstract: extracted.abstract ?? '', theme: extracted.theme ?? '', scope: writeScope(extracted.type), content: extracted.content.slice(0, 80) })
                maybeHousekeeping()
              } catch (err) {
