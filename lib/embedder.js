...
export class RemoteEmbedder {
  constructor({ baseUrl, apiKey, model, dim = 0, cacheSize = 1024, timeoutMs = 30000, fetchImpl }) {
    this.name = 'remote'
...
export class RemoteReranker {
  ...
  async rerank(query, docs) {
    ...
    for (let i = 0; i < docs.length; i++) {
      ...
      if (hit !== undefined) {
        scores.set(i, hit)
        this._cacheTouch(key)   // 严格 LRU：读取刷新位置
    ...
    if (pending.length > 0) {
      ...
      const j = await res.json()
      const covered = new Set() // pending 数组下标（API 返回过分的）
      for (const r of Array.isArray(j.results) ? j.results : []) {
        ...
        scores.set(p.i, s)
        this._cacheSet(p.key, s)
        covered.add(r.index)
      }
      // API 未覆盖的 pending doc 补 0 分（修复：部分缓存命中时不再丢弃任何 doc）
      for (let k = 0; k < pending.length; k++) {
        if (!covered.has(k)) scores.set(pending[k].i, 0)
      }
    }
    // 统一按 docs 原始顺序返回全部（缓存命中与 API 结果合并；调用方按 score 自行排序）
...
export async function createEmbeddingServices(cfg, deps = {}) {
  ...
  // rerank 启用判定：cfg.rerank 存在即启用（由调用方组装时把关 enabled），
  // 但显式 enabled:false 仍尊重（防御未来其它调用方传入整段配置）。
  // ⚠️ 勿改为 cfg.rerank?.enabled——index.js 组装后传给本函数的 rerank 对象是
  // 已剥离开关的 {model, baseUrl, apiKey}（v0.9.25 前的装配 bug：检查 .enabled
  // 恒 undefined → reranker 从未创建，与密钥/开关配置无关）。
  if (cfg.rerank && cfg.rerank.enabled !== false) {
    const rk = cfg.rerank
