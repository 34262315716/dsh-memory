...
export class RemoteReranker {
  constructor({ baseUrl, apiKey, model, timeoutMs = 8000, fetchImpl }) {
    this.name = 'remote'
    this.baseUrl = (baseUrl ?? 'https://api.siliconflow.cn/v1').replace(/\/+$/, '')
    this.apiKey = apiKey
    this.model = model
  ...
  async rerank(query, docs) {
    const res = await this._post('/rerank', { model: this.model, query, documents: docs, top_n: docs.length })
    if (!res.ok) throw new Error('rerank API ' + res.status + ': ' + (await res.text()).slice(0, 120))
    const j = await res.json()
    return (Array.isArray(j.results) ? j.results : []).map((r) => ({ index: r.index, score: r.relevance_score }))
  }
  async _post(path, body) {
    const ctrl = new AbortController()
    const timer = setTimeout(() => ctrl.abort(), this.timeoutMs)
