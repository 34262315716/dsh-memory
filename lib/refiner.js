...
export async function llmStrictJson(ctx, cfg, prompt, system = '你是严格的 JSON 输出器，只输出合法 JSON。', sessionId) {
  ...
  const meta = { attempts: 0, continuations: 0, timedOut: false, truncated: false, reasoningChars: 0, salvaged: false, textChunks: 0, reasoningChunks: 0, otherChunks: 0, lastFinish: null }
  let acc = ''
  let reasoning = ''
  ...
  const call = async (sys, text) => {
    ...
    if (r.finish === 'length') meta.truncated = true
    acc += r.text ?? ''
  ...
  const salvageFromReasoning = () => {
    ...
    try {
      const j = parseJsonLoose(reasoning)
      meta.salvaged = true
      return j
    } catch { return null }
  }
  ...
  let parsed = null
  try { parsed = parseJsonLoose(acc) } catch { /* 交给兜底/续跑/重试 */ }
  ...
  const truncatedLike = () => acc.trim() === '' || meta.timedOut || meta.truncated || acc.includes('{')
  while (meta.continuations < maxCont && truncatedLike()) {
    ...
    try { parsed = parseJsonLoose(acc) } catch { /* 继续 */ }
    if (parsed !== null) return { text: acc, json: parsed, meta }
    parsed = salvageFromReasoning()
    if (parsed !== null) return { text: reasoning, json: parsed, meta }
  ...
  // 纯叙述跑题（没有任何 JSON 结构）→ 强化指令重试一次（v0.9.27）
  if (!acc.includes('{')) {
    await call(RETRY_SYSTEM, `${prompt}\n\n你上一次的输出不是合法 JSON。请重新输出，只给 JSON 本体。`)
    try {
      return { text: acc, json: parseJsonLoose(acc), meta }
    } catch (err) {
...
/** 失败诊断：把"为什么解析不出来"讲清楚（正文段数 / 思考字数 / 其他块 / 末次 finish / 是否被掐断）。 */
function diagnose(reason, meta) {
  return `LLM 输出无法解析为 JSON（${reason}；尝试 ${meta.attempts} 次、续跑 ${meta.continuations} 次；`
    + `正文回包 ${meta.textChunks} 段 / 思考 ${meta.reasoningChars} 字 / 其他块 ${meta.otherChunks} 个；`
    + `末次 finish=${meta.lastFinish}；收到的块类型=[${(meta.chunkTypes ?? []).join(',') || '无'}]${meta.timedOut ? '；曾被时间预算掐断' : ''}）`
}
 ...
 */
const EXTRACT_RULES = `先把判断写清楚，再写记忆条目。

【第一步：analysis（你的判断过程，会被审计，请认真写，不要敷衍）】
用 2-5 句回答：
1. 这一轮实际发生了什么（一句话，只描述事实）
2. 有没有值得长期记住的东西？如果有，是什么性质：决策 / 结论 / 偏好 / 教训 / 关键事实 / 关于用户本人的稳定信息
3. 写的时候必须保住哪些关键约束（数字、版本、路径、边界条件、前提、否定条件）——写漏了会让这条记忆在未来被误用
4. 如果没有任何长期价值（寒暄、确认、纯过程、工具中间输出），直接说明并让 items 为空

【第二步：items（0-3 条，每条独立、自包含、可单独读懂）】
硬性要求：
- content 必须自包含：脱离这轮对话也能看懂，不出现"上面""这次""刚才"这类指代
- content 必须带判据：写清数字/条件/边界/原因。禁止"优化了性能""改进了流程"这类没有信息量的空话
- 不写过程：不写"讨论了""尝试了""改了三版""最终确定"
- 不写元信息：不出现"用户说""助手认为""对话中"等字样
...
- keywords：3-8 个**具体名词实体**（技术栈/模块名/类名/文件名/版本号/专有名词），用于检索与图谱。
  **禁止**：完成/修复/实现/使用/优化/更新 等动作泛词；禁止 本项目名/记忆系统/图谱/插件/DSH 等话题词；
  禁止 现在/什么/这个/没有/不是/一个/问题/用户/主人/继续 等口语虚词；禁止 2 字碎片词。
...
对话：
[用户]
${truncate(userPart, 6000)}

[助手]
${truncate(assistantPart, 6000)}`
