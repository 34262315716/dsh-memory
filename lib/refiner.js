 ...
 * @returns {Promise<{content: string, type: string, layer: string, keywords: string[], aspect: string, abstract: string, theme: string}>}
 */
...
7. abstract ∈ principle | event（抽象层级）
   - principle = 可复用的方法/原则/经验/看法（"怎么看待/怎么做"），如"注入阈值必须与分数量纲对齐"
   - event = 一次性具体事件/产出（"做了什么/发生了什么"），如"v0.9.31 修复了降级迁移 bug"
   - 判断标准：能抽成通用原则的记 principle（即使来自具体事件）；纯事实记录是 event
8. theme：简短稳定的**名词标签**（2-8 字，概括内容所属领域，如"四级备考"/"AI绘画"/"dsh-memory 开发"；
   **不要**句子、动词短语、标点或语气词；确实无法归类时填空字符串 ""）
9. 没有值得记的内容时输出 {"content": ""}

输出严格 JSON（无其他文字、无 markdown 围栏）：
{"content": "...", "type": "decision", "layer": "sm", "keywords": ["..."], "aspect": "", "abstract": "principle", "theme": "dsh-memory 开发"}
...
${truncate(assistantPart, 4000)}`
  ...
  return {
    ...
    // v0.10 abstraction：白名单校验（principle|event），越界回落空串——旧模型/无标签输出不崩
    abstract: parsed.abstract === 'principle' || parsed.abstract === 'event' ? parsed.abstract : '',
    // theme：LLM 名词标签；清洗（去空白、压空格、截 30 字），空串=未归类
    theme: typeof parsed.theme === 'string' ? parsed.theme.trim().replace(/\s+/g, ' ').slice(0, 30) : '',
  }
}
