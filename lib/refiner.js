...
export async function llmStrictJson(ctx, cfg, prompt, system = '你是严格的 JSON 输出器，只输出合法 JSON。', sessionId) {
  ...
  const meta = { attempts: 0, continuations: 0, timedOut: false, truncated: false, reasoningChars: 0, salvaged: false, textChunks: 0, textChars: 0, reasoningChunks: 0, otherChunks: 0, lastFinish: null, maxTokens: Number(ref.maxTokens) || 0, repaired: false }
  let acc = ''
  let reasoning = ''
...
function diagnose(reason, meta) {
  ...
  return `LLM 输出无法解析为 JSON（${reason}；尝试 ${meta.attempts} 次、续跑 ${meta.continuations} 次；`
    + `正文回包 ${meta.textChunks} 段（${meta.textChars ?? 0} 字）/ 思考 ${meta.reasoningChars} 字 / 其他块 ${meta.otherChunks} 个；`
