...
export class MemoryStore {
  ...
  /** 启动迁移（幂等）：旧实体图上的记忆级边（similarTo/before）→ memory_links。 */
  #migrateMemoryLinks() {
    try {
      const n = this.db.prepare('SELECT COUNT(*) AS c FROM memory_links').get().c
      if (n > 0) return
      const rows = this.db.prepare(
   ...
   *  幂等（同三元组 UPSERT 重新激活 + 更新权重）；记忆删除级联清理。返回新建边数（0/1）。 */
  link(fromMemoryId, toMemoryId, type, weight = 1) {
    ...
    const exists = this.db.prepare('SELECT 1 FROM memories WHERE id = ?').get(fromMemoryId)
      && this.db.prepare('SELECT 1 FROM memories WHERE id = ?').get(toMemoryId)
    if (!exists) return 0
