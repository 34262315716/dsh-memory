...
import { suggestLinks, pairKey, LINK_TAU, LINK_TAU_CROSS_SCOPE, LINK_MAX_PER_MEMORY, LINK_TOPK, LINK_LAYERS } from './link-suggest.js'
...
-- 阶段四（v0.9.5）：运行日志——背后运行了什么完全透明可见（写入/注入/检索/巡检/蒸馏）
...
-- v0.9.8：主题聚类增量——簇质心/词频/成员数持久化（避免每次启动全量重聚 + 全量重嵌入）。
-- memories.cluster_id → 标记已归簇（NULL/'' = 待增量处理）。
...
/** v0.9.11：before 演化实体「稀有度」上限——被超过该条记忆共享的 label 视为泛词，不承担演化。
 *  linkBefore 建边与 beforeAudit 审计共用此口径，避免标准漂移。 */
export const RARE_MAX = 8
...
export class MemoryStore {
  ...
  constructor(dbFile, opts = {}) {
    ...
    this.degraded = opts.degraded ?? false      // 降级态（v0.9.31）：配置远程嵌入但初始化失败落到 rule——禁止破坏性迁移
    this.reranker = opts.reranker ?? null       // Reranker seam（可空）
    this.rerankCooldownUntil = 0                // 熔断（v0.9.29）：rerank 失败后 5 分钟内跳过，避免每次检索全量重试挂起
    ...
    try {
      ...
      // v0.10 阶段一（abstraction）：principle（方法/原则/可复用经验）| event（具体事件/产出）
      if (!cols.some((c) => c.name === 'abstract')) {
        this.db.exec("ALTER TABLE memories ADD COLUMN abstract TEXT NOT NULL DEFAULT ''")
        console.log('[dsh-memory] memories 表补列 abstract（principle/event 抽象层级）')
      }
    } catch (err) {
      console.warn('[dsh-memory] theme/profile_aspect 列检测失败: ' + err.message)
    }
    // v0.9.8：主题聚类增量标记列（cluster_id NULL/'' = 待归簇）。
    try {
      const cols2 = this.db.prepare('PRAGMA table_info(memories)').all()
      if (!cols2.some((c) => c.name === 'cluster_id')) {
        this.db.exec("ALTER TABLE memories ADD COLUMN cluster_id TEXT NOT NULL DEFAULT ''")
        console.log('[dsh-memory] memories 表补列 cluster_id（主题聚类增量）')
      }
    } catch (err) {
      console.warn('[dsh-memory] cluster_id 列检测失败: ' + err.message)
    }
    // v0.12.1：归档标记列（管家治理的落点）。
    // 归档 ≠ 删除：被归并的相似记忆、判定为陈旧过程噪音的情景快照都留在这里，
    // 只是不再参与检索/注入（数据还在，可查、可恢复）。老库靠这一列就地升级。
    try {
      const cols3 = this.db.prepare('PRAGMA table_info(memories)').all()
      if (!cols3.some((c) => c.name === 'archived')) {
        this.db.exec('ALTER TABLE memories ADD COLUMN archived INTEGER NOT NULL DEFAULT 0')
        console.log('[dsh-memory] memories 表补列 archived（管家治理：归档不删除）')
      }
    } catch (err) {
      console.warn('[dsh-memory] archived 列检测失败: ' + err.message)
    }
    // v0.13.0：常驻（pinned）标记列——被钉选的记忆**恒定注入**（不走检索、每步随行）。
    // 用于"珍贵教训要一直在场"，而不是等话题撞上才冒出来。老库同样就地升级。
    try {
      const cols4 = this.db.prepare('PRAGMA table_info(memories)').all()
      if (!cols4.some((c) => c.name === 'pinned')) {
        this.db.exec('ALTER TABLE memories ADD COLUMN pinned INTEGER NOT NULL DEFAULT 0')
        console.log('[dsh-memory] memories 表补列 pinned（常驻记忆：恒定注入）')
      }
    } catch (err) {
      console.warn('[dsh-memory] pinned 列检测失败: ' + err.message)
    }
    ...
    try {
      ...
      // 降级态（v0.9.31）：配置远程嵌入但初始化失败落到 rule——**禁止破坏性迁移**，
      // 保留现有表维度并暂停向量路（FTS/关键词照常），恢复后自动回来；否则一次网络
      // 抖动就会 DROP 全表 + 全量重嵌入（9/9 实测：4096→256→4096 反复迁移 + 208 条悬空）。
      const curDim = this.#vectorTableDim()
      // 降级态且维度不一致：暂停向量路（禁止破坏性迁移）。标志在统一重建后
      // 再次覆写 vecEnabled 时使用——否则 334 行无条件置 true 会把暂停覆盖掉。
      const degradedMismatch = !!(this.degraded && this.embedder && curDim !== null && curDim !== this.vecDim)
      if (curDim !== null && curDim !== this.vecDim) {
        if (this.embedder && !this.degraded) {
          this.db.exec('DROP TABLE memory_vectors')
          console.log(`[dsh-memory] 向量维度迁移 ${curDim} → ${this.vecDim}（重建空表，待重嵌入）`)
        } else if (this.embedder) {
          this.vecDim = curDim
          console.warn(`[dsh-memory] embedder 降级态与向量表维度不一致（${this.embedder.name} ${this.vecDim} vs 表 ${curDim}），向量路暂停，恢复后自动重建`)
        } else {
          this.vecDim = curDim
        }
      }
      this.db.exec(`CREATE VIRTUAL TABLE IF NOT EXISTS memory_vectors USING vec0(embedding float[${this.vecDim}])`)
      if (!degradedMismatch) this.vecEnabled = true
    } catch (err) {
      this.vecEnabled = false
   ...
   *  失败批次自动重试（v0.9.31）：网络抖动/超时导致的批次失败进 retry 队列，最多
   *  retryRounds 轮（默认 3 轮，轮间 800ms 退避），尽可能一次启动补完。 */
  async reembedMissing(batchSize = 16, { retryRounds = 3, retryDelayMs = 800 } = {}) {
    ...
    for (let round = 0; round <= retryRounds && pending.length > 0; round++) {
      if (round > 0) await sleep(retryDelayMs * (2 ** (round - 1))) // 指数退避：800 / 1600 / 3200
      const next = []
      for (let i = 0; i < pending.length; i += batchSize) {
        ...
        try {
          ...
          // 失败批次恒进 next：重试轮内会重跑，最后一轮则作为 pending 留下（v0.9.31 修正：
          // 此前最后一轮失败批次不保留，返回的 pending 谎报 0，等于"放弃"的条目被静默吞掉）
          next.push(...batch)
          if (round < retryRounds) continue
          console.warn(`[dsh-memory] 重嵌入批次失败（重试 ${round} 轮后放弃 ${batch.length} 条）: ${err.message}`)
          continue
        }
      ...
      if (next.length > 0 && round < retryRounds) console.log(`[dsh-memory] 重嵌入批次失败 ${next.length} 条进入重试（第 ${round + 2} 轮）`)
      pending = next
    }
    return { done, pending: pending.length }
  }
   ...
   */
  /** 读取已持久化主题簇（theme_clusters → 内存对象）。 */
  #loadThemeClusters() {
    const out = []
    for (const r of this.db.prepare('SELECT * FROM theme_clusters ORDER BY updated_at').all()) {
      let centroid = []
      let keywords = {}
      try { centroid = JSON.parse(r.centroid) } catch { centroid = [] }
      try { keywords = JSON.parse(r.keywords) } catch { keywords = {} }
      if (!Array.isArray(centroid)) centroid = []
      out.push({
        id: r.id,
        label: r.label,
        centroid,
        keywords: new Map(Object.entries(keywords)),
  ...
  /** 主题簇质心维度与当前 embedder 不一致（rule↔remote 迁移）→ 簇失效，需清空重聚。 */
  #themeClustersPointless(clusters) {
    return clusters.some((c) => c.centroid.length > 0 && c.centroid.length !== this.vecDim)
  }

  /**
   * 主题聚类（记忆级）——**增量**：仅处理未归簇记忆（memories.cluster_id 为空），
   * 并入已持久化簇（质心加权更新 + 词频累计）或新建簇；已归簇记忆不再重聚/重嵌入。
   * 簇状态落 theme_clusters 表（centroid/keywords/member_count），跨重启增量续跑。
   * `incremental=false` = 全量重聚（清空簇表 + 全部重归，用于维度迁移/手动重建）。
   * @param {number} threshold 余弦凝聚阈值（默认 0.78）
   * @param {{incremental?: boolean}} [opts]
   * @returns {Promise<{label: string, members: string[]}[]>}
   */
  async themeMemories(threshold = 0.78, { incremental = true } = {}) {
    let clusters = incremental ? this.#loadThemeClusters() : []
    const pointless = incremental && this.#themeClustersPointless(clusters)
    let rows
    if (incremental && !pointless) {
      rows = this.db.prepare(
        "SELECT id, content, keywords FROM memories WHERE cluster_id IS NULL OR cluster_id = '' ORDER BY rowid",
      ).all()
    } else {
      // 全量模式 或 维度迁移（含"全部已归簇但簇维度失效"场景）：清空簇与归属标记，本批全部重归
      this.db.exec('DELETE FROM theme_clusters')
      this.db.exec("UPDATE memories SET cluster_id = ''")
      clusters = []
      rows = this.db.prepare('SELECT id, content, keywords FROM memories ORDER BY rowid').all()
    }
    ...
    // 主题标签：簇内关键词高频（非停用词）top2；频次 ≥2 才入选（单成员簇碎片词恒为 1 → label 空）
    const labelOf = (kw) => [...kw.entries()]
      .filter(([, c]) => c >= 2)
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      let words = []
      try { words = JSON.parse(rows[i].keywords) } catch { words = [] }
      let cls
      if (best >= 0 && bestSim >= threshold) {
        cls = clusters[best]
        const n = cls.memberCount
        for (let d = 0; d < cls.centroid.length; d++) cls.centroid[d] = (cls.centroid[d] * n + v[d]) / (n + 1)
        cls.memberCount++
      } else {
        cls = { id: 'tc-' + randomUUID().slice(0, 8), label: '', centroid: v.slice(), keywords: new Map(), memberCount: 1, members: [] }
        clusters.push(cls)
      }
      ...
      for (const w of words) {
        ...
        cls.keywords.set(k, (cls.keywords.get(k) ?? 0) + 1)
      }
      cls.label = labelOf(cls.keywords)
      cls.members.push(rows[i].id)
      touched.add(cls.id)
    }
    // 持久化：簇表 upsert + 记忆 cluster_id/theme 回写
    const upsCl = this.db.prepare('INSERT INTO theme_clusters (id, label, centroid, keywords, member_count, updated_at) VALUES (?, ?, ?, ?, ?, ?) ON CONFLICT(id) DO UPDATE SET label = excluded.label, centroid = excluded.centroid, keywords = excluded.keywords, member_count = excluded.member_count, updated_at = excluded.updated_at')
    const updMem = this.db.prepare('UPDATE memories SET theme = ?, cluster_id = ? WHERE id = ?')
    const now = Date.now()
    this.db.exec('BEGIN IMMEDIATE')
    try {
      for (const cls of clusters) {
        if (!touched.has(cls.id)) continue
        upsCl.run(cls.id, cls.label, JSON.stringify(cls.centroid), JSON.stringify(Object.fromEntries(cls.keywords)), cls.memberCount, now)
      }
      for (const m of rows) {
        const cls = clusters.find((c) => (c.members ?? []).includes(m.id))
        if (cls) updMem.run(cls.label, cls.id, m.id)
      }
      this.db.exec('COMMIT')
    } catch (err) {
      this.db.exec('ROLLBACK')
    ...
    return clusters.map((c) => ({ label: c.label, members: c.members ?? [] }))
  }

  #prepare() {
    this.stmt = {
      insertMemory: this.db.prepare(
        'INSERT INTO memories (id, layer, type, scope, content, keywords, strength, last_access, created_at, updated_at, abstract, theme, profile_aspect) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
      ),
  ...
  /** 新增一条记忆。返回 id。 */
  async add({ layer = 'sm', type = 'note', scope = 'global', content, keywords = [], strength = 1, aspect = '', abstract = '', theme = '' }) {
    const id = `mem-${randomUUID().slice(0, 8)}`
    const now = Date.now()
    const kwJson = JSON.stringify([...new Set(keywords)])
    // 白名单兜底（v0.10 abstraction）：越界值回落空串，不落非法标记
    const abs = abstract === 'principle' || abstract === 'event' ? abstract : ''
    ...
    this.db.exec('BEGIN IMMEDIATE')
    try {
      this.stmt.insertMemory.run(id, layer, type, scope, content, kwJson, strength, now, now, now, abs, theme ?? '', aspect)
  ...
  list({ scope, layer, type, limit = 50, includeArchived = false, pinned } = {}) {
    const where = []
    const args = []
    // v0.12.1：归档记忆默认不出现在浏览/注入结果里（需显式 includeArchived 才带出）
  ...
  /**
   * 常驻（pinned）记忆清单（v0.13.0）：恒定注入通道的数据源。
   * 顺序用 **rowid（钉选先后）** 而不是 updated_at——注入块必须是确定性的，
   * 否则块指纹每步都在变，去抖与 KV 缓存友好同时失效。
   * 不带 scope 过滤：钉选是**人工裁定**，谁被钉上就是要跨项目一直在场。
   */
  listPinned({ limit = 20 } = {}) {
    const rows = this.db.prepare('SELECT * FROM memories WHERE pinned = 1 AND archived = 0 ORDER BY rowid LIMIT ?').all(limit)
    return rows.map((r) => ({ ...r, keywords: JSON.parse(r.keywords) }))
  }

  pinnedCount() {
    return this.db.prepare('SELECT COUNT(*) AS n FROM memories WHERE pinned = 1 AND archived = 0').get().n
  }

  /** 钉选/取消钉选。刻意不动 updated_at：钉选是治理动作，不该把记忆顶到浏览列表最前面。 */
  setPinned(id, on = true) {
    return this.db.prepare('UPDATE memories SET pinned = ? WHERE id = ?').run(on ? 1 : 0, id).changes > 0
  }
  ...
  /** FTS5 + 关键词 + 子串三路检索（阶段一；向量 RRF 属阶段二）。scope 支持数组（多 scope IN 匹配）。 */
  async search(query, { scope, limit = 8, minScore = 0, excludeIds = [], boost = {} } = {}) {
    const scopes = scope ? (Array.isArray(scope) ? scope : [scope]) : null
    const scopeMatch = scopes ? (s) => scopes.includes(s) : () => true
    const qTokens = tokenize(query)
    const excluded = new Set(excludeIds)
    // v0.12.1 归档过滤：被管家归档的记忆（已归并进别的记忆 / 判定为陈旧过程噪音）
    // 不参与任何一路检索——三路共用这一个集合，避免"归档了还从向量路冒出来"。
    // 归档量通常几十条，一次查询可接受；老库尚未迁移出该列时退化为不过滤（不抛错）。
    let archivedSet = new Set()
    try {
      archivedSet = new Set(this.db.prepare('SELECT id FROM memories WHERE archived = 1').all().map((r) => r.id))
    } catch { /* 无 archived 列 → 不过滤 */ }
    ...
    if (ftsTerms) {
      try {
        for (const row of this.stmt.ftsSearch.all(ftsTerms, 50)) {
          if (excluded.has(row.id) || archivedSet.has(row.id)) continue
          if (!scopeMatch(row.scope)) continue
          // bm25 为负值（越小越好）→ 转正分
    ...
    if (qTokens.size > 0 || likeQuery) {
      const rows = scopes
        ? this.db.prepare(`SELECT * FROM memories WHERE scope IN (${scopes.map(() => '?').join(',')})`).all(...scopes)
        : this.db.prepare('SELECT * FROM memories').all()
      for (const r of rows) {
        if (excluded.has(r.id) || archivedSet.has(r.id)) continue
    ...
    if (this.vecEnabled) {
      try {
        ...
        const rowidScope = new Map()
        for (const r of rows) {
          const mem = this.db.prepare('SELECT id, scope FROM memories WHERE rowid = ?').get(r.rowid)
          if (mem) { rowidToId.set(r.rowid, mem.id); rowidScope.set(r.rowid, mem.scope) }
        }
        vecHits = rows
          .filter((r) => rowidToId.has(r.rowid)
            && !excluded.has(rowidToId.get(r.rowid))
            && !archivedSet.has(rowidToId.get(r.rowid))   // v0.12.1：归档不进向量路
            && scopeMatch(rowidScope.get(r.rowid)))   // 向量路 scope 过滤（v0.9.4 分层后必须）
    ...
    // 类型加权 boost（P0.1 画像召回）：按记忆 type 放大排序分——画像类 content 短/关键词少，
    // 在 RRF 里常被泛词长记忆碾压（"查丹道记录"注入回图谱治理记录的根因）。
    // v0.10 abstraction 维度：boost 同时支持 abstract 键（principle/event），
    // 权重 = typeBoost × abstractBoost——注入路径传 { profile:3, principle:1.5, event:0.7 }：
    // principle（方法/原则）优先、event（具体事件）降权（强相关才注入）。
    // 加权先于 rerank 候选选取：被抬升的画像才有机会进精排候选与 minScore 门槛。
    // 仅影响调用方显式传入的路径（注入）；memory_search 工具不传 → 零行为变化。
    if (boost && Object.keys(boost).length > 0 && scored.length > 0) {
      const typeMap = new Map()
      const absMap = new Map()
      const ids = scored.map((h) => h.id)
      for (let i = 0; i < ids.length; i += 200) {
        const chunk = ids.slice(i, i + 200)
        const rows = this.db.prepare(`SELECT id, type, abstract FROM memories WHERE id IN (${chunk.map(() => '?').join(',')})`).all(...chunk)
        for (const r of rows) { typeMap.set(r.id, r.type); absMap.set(r.id, r.abstract) }
      }
      for (const h of scored) {
        const tw = Number(boost[typeMap.get(h.id)])
        const aw = Number(boost[absMap.get(h.id)])
        const w = (Number.isFinite(tw) ? tw : 1) * (Number.isFinite(aw) ? aw : 1)
        if (!Number.isFinite(w) || w === 1) continue // NaN/非数防御：视为不加权
        h.score *= w
      }
      scored.sort((a, b) => b.score - a.score || a.id.localeCompare(b.id))
    }
    // minScore 门槛（v0.9.29 前移——v0.9.26 激活 rerank 后融合分尺度变为 [0,1]，
    // 与原 RRF 量纲 ~0.05 错配：按融合分过滤会让噪音候选（rr 0.05 融合后 0.69）漏网，
    // minScore 质量旋钮失灵，镜像 v0.8.3 量纲失配教训）。门槛语义 = 「召回相关证据」，
    // 应在 rerank 之前按（加权）RRF 分过滤一次；rerank 只负责精排不改召回。
    let passGate = scored
    if (minScore > 0) {
      passGate = scored.filter((h) => h.score >= minScore)
    }
    // reranker 后置精排（阶段三④期4）：RRF topK 候选 → 重排 → 融合分；失败/关闭 → 原 RRF 顺序（零损失）。
    // 熔断（v0.9.29）：失败置 5 分钟冷却，期间跳过 rerank，避免每次检索 8s 超时挂起 + warn 刷屏
    if (this.reranker && passGate.length >= this.rerankMinCandidates && Date.now() >= this.rerankCooldownUntil) {
      try {
        const top = passGate.slice(0, this.rerankTopK)
        const docs = []
        const docIds = []
        for (const h of top) {
        ...
        if (docs.length >= this.rerankMinCandidates) {
          ...
          const maxRrf = Math.max(...passGate.map((h) => h.score), 1e-9)
          for (const h of passGate) {
          ...
          passGate.sort((a, b) => b.score - a.score || a.id.localeCompare(b.id))
        }
      } catch (err) {
        this.rerankCooldownUntil = Date.now() + 5 * 60 * 1000
        console.warn(`[dsh-memory] reranker 失败（熔断 5 分钟），降级 RRF 顺序: ${err.message}`)
      }
    }
    const out = []
    for (const h of passGate) {
  ...
  /**
   * v0.9.12 关键词→实体稀有化：从候选关键词里挑"库内稀有的具体实体"。
   * 过滤停用词/碎片；剔除已被超过 rareMax 条记忆共享的高频泛词（如 dsh-memory×17）；
   * 按稀有度（共享数升序）排序取前 k 个——graphLink 建图与 before/mentions 统一走此口径，
   * 泛词不再进图（治本：LLM 关键词若泛，规则侧硬兜底）。
   * @param {string[]} entityLabels 候选关键词
   * @param {{k?:number, rareMax?:number}} [opts]
   * @returns {string[]} 稀有实体标签
   */
  pickRareEntities(entityLabels, { k = 8, rareMax = RARE_MAX } = {}) {
    const seen = new Set()
    const clean = []
    for (const l of entityLabels ?? []) {
      if (!l || String(l).length <= 1) continue
      if (GRAPH_STOP_WORDS.has(String(l).toLowerCase())) continue
      if (seen.has(l)) continue
      seen.add(l)
      clean.push(l)
    }
    if (clean.length === 0) return []
    // 稀有度：nodes 中该 label 当前被多少记忆共享（0 = 尚未建过，最稀有）
    const cnt = new Map()
    for (const r of this.db.prepare("SELECT label, COUNT(DISTINCT memory_id) c FROM nodes WHERE kind = 'entity' GROUP BY label").all()) {
      cnt.set(r.label, r.c)
    }
    return clean
      .filter((l) => (cnt.get(l) ?? 0) <= rareMax)
      .sort((a, b) => (cnt.get(a) ?? 0) - (cnt.get(b) ?? 0))
      .slice(0, k)
  }

  /** 图谱：为一条记忆建 entity 节点 + 同记忆内全连接边（幂等：同 label+记忆 → 同节点/边）。
   *  v0.9.12：实体稀有化——只建"库内稀有"的具体实体（高频泛词不进图），按稀有度取前 k。 */
  graphLink(memoryId, entityLabels, edgeType = 'mentions') {
    // 实体过滤：稀有化（去停用词/碎片 + 剔高频泛词 + 稀有度排序）
    const labels = this.pickRareEntities(entityLabels)
   ...
   * v0.9.11 收紧（治理"假演化"）：不是"共享任意词就串链"，而是要求**演化关系**才连——
   *   - 实体 label 须为"稀有实体"（被 ≤ RARE_MAX 条记忆共享；超高频泛词不承担演化）
   *   - 候选须 与当前记忆共享 ≥2 个稀有实体 或 同主题（非空）
   * 聚合全部候选后按时间取最近前驱/后继各连一条（每方向至多 1 条，取代 per-label 重复链）。
   */
  linkBefore(memoryId) {
    // RARE_MAX：被超过 8 条记忆共享的 label 视为泛词，不承担演化（与 beforeAudit 共用模块常量）
    const mem = this.db.prepare('SELECT id, created_at, theme FROM memories WHERE id = ?').get(memoryId)
    if (!mem) return 0
    // 该记忆的实体 label 集合（归一化节点经 node_memories；否则 fallback nodes.memory_id）
    let labels = this.db.prepare(
    ...
    if (labels.length === 0) {
      labels = this.db.prepare("SELECT DISTINCT label FROM nodes WHERE memory_id = ? AND kind = 'entity'").all(memoryId).map((r) => r.label)
    }
    if (labels.length === 0) return 0
    // label 稀有度：被 ≤ RARE_MAX 条记忆共享才可能是"演化实体"
    const labelCnt = new Map()
    for (const r of this.db.prepare("SELECT label, COUNT(DISTINCT memory_id) c FROM nodes WHERE kind = 'entity' GROUP BY label").all()) {
      labelCnt.set(r.label, r.c)
    }
    // 候选集：共享任一稀有 label 的记忆（累计共享数）+ created_at
    const cand = new Map()
    const peersQ = this.db.prepare(
      `SELECT DISTINCT nm.memory_id AS id, m.created_at FROM node_memories nm
       JOIN memories m ON m.id = nm.memory_id JOIN nodes n2 ON n2.id = nm.node_id
       WHERE n2.label = ? AND n2.kind = 'entity' AND nm.memory_id != ? ORDER BY m.created_at, nm.memory_id`)
    const peersQFb = this.db.prepare(
    ...
    for (const label of labels) {
      if ((labelCnt.get(label) ?? 0) > RARE_MAX) continue
      let peers = peersQ.all(label, memoryId)
      if (peers.length === 0) peers = peersQFb.all(label, memoryId)
      for (const p of peers) {
        const c = cand.get(p.id) ?? { created_at: p.created_at, shared: 0 }
        c.shared++
        cand.set(p.id, c)
      }
    }
    if (cand.size === 0) return 0
    // 演化判定：共享 ≥2 稀有实体 或 同主题（非空）
    const themeQ = this.db.prepare('SELECT theme FROM memories WHERE id = ?')
    const ok = []
    for (const [id, c] of cand) {
      if (c.shared >= 2) { ok.push({ id, created_at: c.created_at }); continue }
      const t = themeQ.get(id)?.theme
      if (t && mem.theme && t === mem.theme) ok.push({ id, created_at: c.created_at })
    }
    if (ok.length === 0) return 0
    ok.sort((a, b) => a.created_at - b.created_at)
    let pred = null
    for (const p of ok) { if (p.created_at <= mem.created_at) pred = p; else break }
    const succ = ok.find((p) => p.created_at > mem.created_at) ?? null
    let n = 0
  ...
  /** 读原始向量（vec0 里的 embedding 可能是 JSON 文本或 float32 blob）。 */
  vecRawOf(rowid) {
    const r = this.db.prepare('SELECT embedding FROM memory_vectors WHERE rowid = ?').get(rowid)
    if (!r) return null
    const v = r.embedding
    if (typeof v === 'string') {
      try { return Float32Array.from(JSON.parse(v)) } catch { return null }
    }
    if (v instanceof Uint8Array) return new Float32Array(v.buffer, v.byteOffset, v.byteLength / 4)
    return null
  }

  /** KNN（sqlite-vec，L2 序）：返回 rowid 列表。 */
  knnRowids(vec, k) {
    try {
      return this.stmt.vecSearch.all(JSON.stringify(Array.from(vec)), k).map((r) => Number(r.rowid))
    } catch {
      return []
    }
  }
  ...
  /**
   * 语义连边（v0.11.0）：写入时用**已有向量**把新记忆连回主图——**纯算法，不调 LLM**。
   * 规则单一来源于 lib/link-suggest.js（阈值 / 互为最近邻 / 跨 scope 守卫 / 度数上限），
   * 这里只负责取数：sqlite-vec KNN 取候选 → 精确余弦复核 → 逐候选验"互为最近邻" → 落边。
   * @returns {number} 新建边数（0 = 没有合格的语义邻居）
   */
  linkSemantic(memoryId, opts = {}) {
    if (!this.vecEnabled) return 0
    const tau = opts.tau ?? LINK_TAU
    const tauCrossScope = opts.tauCrossScope ?? LINK_TAU_CROSS_SCOPE
    const maxLinks = opts.maxLinks ?? LINK_MAX_PER_MEMORY
    const topK = opts.topK ?? LINK_TOPK
    const layers = opts.layers === undefined ? LINK_LAYERS : opts.layers
    const self = this.db.prepare('SELECT rowid, id, layer, scope, created_at FROM memories WHERE id = ?').get(memoryId)
    if (!self) return 0
    const selfVec = this.vecRawOf(Number(self.rowid))
    if (!selfVec) return 0
    const candRowids = this.knnRowids(selfVec, topK + 4).filter((r) => r !== Number(self.rowid))
    if (candRowids.length === 0) return 0
    // 精确余弦复核（KNN 的距离是 L2，且不保证向量严格归一化）
    const scored = []
    for (const r of candRowids) {
      const v = this.vecRawOf(r)
      if (v) scored.push({ rowid: r, sim: cosine(selfVec, v), vec: v })
    }
    scored.sort((a, b) => b.sim - a.sim)
    const picked = scored.slice(0, topK)
    if (picked.length === 0) return 0
    // rowid → 记忆 id 全表映射：KNN 返回的 rowid 可能属于**任何**记忆（不只被选中的候选），
    // 早前只映射候选集会让"互为最近邻"永远判不出来（v0.11.0 集成测试抓到的 bug）
    const idByRowid = new Map(this.db.prepare('SELECT rowid, id FROM memories').all().map((r) => [Number(r.rowid), r.id]))
    const metaByRowid = new Map()
    for (const p of picked) {
      const m = this.db.prepare('SELECT rowid, id, layer, scope, created_at FROM memories WHERE rowid = ?').get(p.rowid)
      if (m) metaByRowid.set(p.rowid, m)
    }
    // 每条候选的 KNN 名次（供"互为最近邻"判定）
    const rankByOwner = new Map()
    rankByOwner.set(self.id, new Map(picked.map((p, i) => [metaByRowid.get(p.rowid)?.id, i])))
    for (const p of picked) {
      const owner = metaByRowid.get(p.rowid)
      if (!owner) continue
      const list = this.knnRowids(p.vec, topK)
      rankByOwner.set(owner.id, new Map(list.map((r, i) => [idByRowid.get(r) ?? r, i])))
    }
    const memories = [self, ...[...metaByRowid.values()]].map((m) => ({ id: m.id, layer: m.layer, scope: m.scope }))
    const neighbors = new Map([[self.id, picked.map((p) => ({ id: metaByRowid.get(p.rowid)?.id, sim: p.sim })).filter((c) => c.id)]])
    const existing = new Set()
    for (const r of this.db.prepare('SELECT from_memory f, to_memory t FROM memory_links WHERE valid_to IS NULL AND (from_memory = ? OR to_memory = ?)').all(memoryId, memoryId)) {
      existing.add(pairKey(r.f, r.t))
    }
    const { edges } = suggestLinks({
      memories,
      neighbors,
      rankOf: (aId, bId) => rankByOwner.get(aId)?.get(bId) ?? Infinity,
      existing,
      opts: { tau, tauCrossScope, maxLinks, topK, layers },
    })
    let n = 0
    for (const e of edges) {
      try { n += this.linkMemories(e.from, e.to, e.type, e.weight) } catch { /* 落边失败不影响主流程 */ }
    }
    return n
  }

  /**
   * v0.9.11：before 边质量审计/清理——逐条评估活跃 before 边是否满足"演化判定"
   * （与 linkBefore 同门槛：共享 ≥2 稀有实体 或 同主题非空）。
   * 默认 dryRun 只报告；apply=true 时把不合格边 valid_to 置位（历史保留、图里消失）。
   * @returns {{kept:number, removed:number, removable:{from,to,reason}[]}}
   */
  beforeAudit({ rareMax = RARE_MAX, apply = false } = {}) {
    const rows = this.db.prepare("SELECT from_memory, to_memory FROM memory_links WHERE type='before' AND valid_to IS NULL").all()
    if (rows.length === 0) return { kept: 0, removed: 0, removable: [] }
    const labelMap = new Map()
    for (const r of this.db.prepare("SELECT memory_id, label FROM nodes WHERE kind = 'entity'").all()) {
      if (!labelMap.has(r.memory_id)) labelMap.set(r.memory_id, new Set())
      labelMap.get(r.memory_id).add(r.label)
    }
    const labelCnt = new Map()
    for (const r of this.db.prepare("SELECT label, COUNT(DISTINCT memory_id) c FROM nodes WHERE kind = 'entity' GROUP BY label").all()) {
      labelCnt.set(r.label, r.c)
    ...
    for (const r of this.db.prepare('SELECT id, theme FROM memories').all()) themeOf.set(r.id, r.theme)
    const kept = [], removed = []
    for (const e of rows) {
      const la = labelMap.get(e.from_memory) ?? new Set()
      const lb = labelMap.get(e.to_memory) ?? new Set()
      let rareShared = 0
      for (const l of la) if (lb.has(l) && (labelCnt.get(l) ?? 0) <= rareMax) rareShared++
      const tF = themeOf.get(e.from_memory), tT = themeOf.get(e.to_memory)
      const sameTheme = Boolean(tF && tT && tF === tT && tF !== '')
      if (rareShared >= 2 || sameTheme) kept.push(e)
      else removed.push({
        from: e.from_memory, to: e.to_memory,
        reason: `共享稀有实体 ${rareShared}（<2）· 主题 ${tF || '∅'} / ${tT || '∅'}`,
      })
    }
    if (apply && removed.length > 0) {
      const deact = this.stmt.memoryLinkDeactivate
      this.db.exec('BEGIN IMMEDIATE')
      try {
        for (const e of removed) deact.run(Date.now(), e.from, e.to, 'before')
        this.db.exec('COMMIT')
      } catch (err) { this.db.exec('ROLLBACK'); throw err }
    }
    return { kept: kept.length, removed: removed.length, removable: removed.slice(0, 50) }
  }

  /**
   * v0.9.13：把"假演化"before 边**归到正确类型**（而不是一律删除）——
   *   - 共享 ≥1 稀有实体 → 改判为 **mentions**（实体共现，weight = 共享稀有实体数）
   *   - 一条稀有实体都不共享（纯泛词沾边）→ 真·无关 → 去激活（before 停用）
   *   - 真演化（rareShared ≥2 或 同主题非空）→ 保持不变
   * dryRun 只报告分类；apply 时：改判边 before 停用 + mentions UPSERT（同向幂等），无关边 before 停用。
   * @returns {{beforeKept:number, toMentions:{from,to,weight}[], toRemove:{from,to}[]}}
   */
  reclassifyBefore({ rareMax = RARE_MAX, apply = false } = {}) {
    const rows = this.db.prepare("SELECT from_memory, to_memory FROM memory_links WHERE type='before' AND valid_to IS NULL").all()
    if (rows.length === 0) return { beforeKept: 0, toMentions: [], toRemove: [] }
    const labelMap = new Map()
    for (const r of this.db.prepare("SELECT memory_id, label FROM nodes WHERE kind = 'entity'").all()) {
      if (!labelMap.has(r.memory_id)) labelMap.set(r.memory_id, new Set())
      labelMap.get(r.memory_id).add(r.label)
    ...
    const labelCnt = new Map()
    for (const r of this.db.prepare("SELECT label, COUNT(DISTINCT memory_id) c FROM nodes WHERE kind = 'entity' GROUP BY label").all()) labelCnt.set(r.label, r.c)
    const themeOf = new Map()
    for (const r of this.db.prepare('SELECT id, theme FROM memories').all()) themeOf.set(r.id, r.theme)
    const toMentions = [], toRemove = []
    let beforeKept = 0
    for (const e of rows) {
      const la = labelMap.get(e.from_memory) ?? new Set()
      const lb = labelMap.get(e.to_memory) ?? new Set()
      ...
      const tF = themeOf.get(e.from_memory), tT = themeOf.get(e.to_memory)
      const sameTheme = Boolean(tF && tT && tF === tT && tF !== '')
      if (rareShared >= 2 || sameTheme) { beforeKept++ }
      else if (rareShared >= 1) { toMentions.push({ from: e.from_memory, to: e.to_memory, weight: rareShared }) }
      else { toRemove.push({ from: e.from_memory, to: e.to_memory }) }
    }
    if (apply) {
      const deact = this.stmt.memoryLinkDeactivate
      const upsert = this.stmt.memoryLinkUpsert
      this.db.exec('BEGIN IMMEDIATE')
      try {
        for (const m of toMentions) {
          const now = Date.now()
          deact.run(now, m.from, m.to, 'before')
          upsert.run(m.from, m.to, 'mentions', m.weight, now)
        }
        for (const r of toRemove) deact.run(Date.now(), r.from, r.to, 'before')
        this.db.exec('COMMIT')
      } catch (err) { this.db.exec('ROLLBACK'); throw err }
    }
    return { beforeKept, toMentions, toRemove }
  }
  ...
  /** 主题簇列表（含成员 id，按成员数降序）——主题治理（v0.10.4 theme relabel）用。
   * 向量聚类给出的 label 是簇内高频词的 top2 拼接（易出碎片词/糊团），本方法供
   * LLM 重命名前读取簇与归属；minMembers 过滤单成员簇（其 label 本就是空串）。 */
  themeClusterList(minMembers = 1) {
    const cls = this.db.prepare(
      'SELECT id, label, member_count AS memberCount FROM theme_clusters WHERE member_count >= ? ORDER BY member_count DESC, updated_at DESC',
    ).all(minMembers)
    const memStmt = this.db.prepare('SELECT id FROM memories WHERE cluster_id = ? ORDER BY rowid')
    return cls.map((c) => ({ ...c, members: memStmt.all(c.id).map((r) => r.id) }))
  }

  /** 重命名主题簇：写回簇标签 + 该簇全部成员的 theme。返回受影响记忆数。
   *  只动 theme/label，不改 cluster_id/质心——增量聚类（themeMemories incremental）
   *  跳过已归簇记忆，故不会被下次聚类覆写。 */
  retagTheme(clusterId, label) {
    const now = Date.now()
    this.db.exec('BEGIN IMMEDIATE')
    try {
      this.db.prepare('UPDATE theme_clusters SET label = ?, updated_at = ? WHERE id = ?').run(label, now, clusterId)
      const n = this.db.prepare('UPDATE memories SET theme = ? WHERE cluster_id = ?').run(label, clusterId).changes
      this.db.exec('COMMIT')
      return n
    } catch (err) {
      this.db.exec('ROLLBACK')
  ...
  decayExpired(now = Date.now(), decayRatePerDay = 0.15) {
    const rows = this.db.prepare(
      'SELECT id, strength, last_access FROM memories WHERE last_access < ? AND archived = 0',
  ...
  // ---------- v0.12.1：管家治理（归档 / 恢复 / 归并）----------
  //
  // 设计取舍：治理动作**一律走「归档」而不是「删除」**——被归并的相似记忆、判定为陈旧
  // 过程噪音的情景快照都留在库里，只是不再参与检索与注入（数据、世界线版本链、图谱边全部保留）。
  // 这样"整合"这件事第一次变得可回退：做错了能捞回来。

  /** 归档记忆（移出检索/注入，不删除）。返回实际归档条数。 */
  archiveMemories(ids, reason = '') {
    const list = (ids ?? []).filter(Boolean)
    if (list.length === 0) return 0
    const stmt = this.db.prepare('UPDATE memories SET archived = 1, updated_at = ? WHERE id = ? AND archived = 0')
    const now = Date.now()
    let n = 0
    this.db.exec('BEGIN IMMEDIATE')
    try {
  ...
  /** 恢复归档（治理可回退的另一半）。返回实际恢复条数。 */
  restoreMemories(ids) {
    const list = (ids ?? []).filter(Boolean)
    if (list.length === 0) return 0
    const stmt = this.db.prepare('UPDATE memories SET archived = 0, updated_at = ? WHERE id = ? AND archived = 1')
    const now = Date.now()
    let n = 0
    this.db.exec('BEGIN IMMEDIATE')
    try {
  ...
  /**
   * 归并：把 sourceIds 的内容并入 targetId，源记忆转为**归档**。
   *
   * 与旧「合并」的区别（v0.12.1 纠偏）：
   *   旧实现把两条内容用 `\n---\n` 拼起来 —— 越拼越长（库内实测最长 9099 字、同一个"早上好啊"
   *   被拼了三遍），那叫堆叠不叫整合。现在：内容由调用方（LLM 归并器）给出**一条更完整的结论**，
   *   没给就保留 target 原文；源记忆归档，不再重复出现在检索里。
   *
   * @returns {Promise<{id, revision, archived, keywords}>}
   */
  async mergeMemories(targetId, sourceIds, { content, keywords, theme, abstract } = {}) {
    const target = this.get(targetId)
    if (!target) throw new Error(`归并目标不存在: ${targetId}`)
    const sources = (sourceIds ?? []).map((id) => this.get(id)).filter(Boolean).filter((m) => m.id !== targetId)
    const mergedKeywords = [...new Set([
      ...(keywords ?? []),
      ...target.keywords,
      ...sources.flatMap((s) => s.keywords),
    ])].slice(0, 60)
    const nextContent = typeof content === 'string' && content.trim() ? content.trim() : target.content
    const r = await this.update(targetId, { content: nextContent, keywords: mergedKeywords, strengthDelta: 0.3 })
    // 维度（主题/抽象层级）：归并结论带来的新标签优先，缺省保留原值
    try {
      this.db.prepare(
        `UPDATE memories SET theme = CASE WHEN ? <> '' THEN ? ELSE theme END,
                             abstract = CASE WHEN ? <> '' THEN ? ELSE abstract END
         WHERE id = ?`,
      ).run(theme ?? '', theme ?? '', abstract ?? '', abstract ?? '', targetId)
    } catch { /* 维度更新失败不影响归并 */ }
    const archived = this.archiveMemories(sources.map((s) => s.id), 'merged')
    return { id: targetId, revision: r.revision, archived, keywords: mergedKeywords }
  }

  /** 治理统计：活跃 vs 归档。 */
  archivedStat() {
    const archived = this.db.prepare('SELECT COUNT(*) c FROM memories WHERE archived = 1').get().c
    const total = this.db.prepare('SELECT COUNT(*) c FROM memories').get().c
    return { archived, total, active: total - archived }
  }

  /** 归档记忆清单（供查看与恢复；按归档时间倒序）。 */
  archivedList(limit = 50) {
    return this.db.prepare(
      `SELECT id, layer, type, scope, content, created_at, updated_at FROM memories
       WHERE archived = 1 ORDER BY updated_at DESC LIMIT ?`,
    ).all(limit)
  }

  /** 陈旧情景快照候选（ep 层 + 超 N 天 + **从未被检索命中** + 强度未升过）。
   *
   * v0.12.3 修正判据 bug：旧实现写的是 `last_access = 0`，而 `add()` 建库时就把 last_access
   * 设成了创建时间——**该条件对新建记忆恒不成立**，于是"陈旧 ep 候选"永远是 0 条
   * （2026-09-21 实测：601 条 ep 里 0 条入选，其中 503 条还是"任务/结果"式规则路径产物）。
   * 正确判据是 `last_access <= created_at`：被检索命中过的记忆会被 touch，last_access 大于创建时间。 */
  epArchiveCandidates(days = 45, limit = 500) {
    const cutoff = Date.now() - days * 24 * 3600 * 1000
    return this.db.prepare(
      `SELECT id, content, created_at, last_access, strength FROM memories
       WHERE layer = 'ep' AND archived = 0 AND created_at < ?
         AND last_access <= created_at AND strength < 1.5
  ...
  async dedupScan(threshold = 0.92, limit = 20) {
    const rows = this.db.prepare("SELECT id, content FROM memories WHERE layer = 'sm' AND archived = 0 ORDER BY rowid").all()
  ...
  agingReport(days = 30, limit = 20) {
    ...
    return this.db.prepare(
      `SELECT id, layer, type, content, strength, last_access, created_at FROM memories
       WHERE archived = 0 AND created_at < ? AND last_access < ? ORDER BY strength ASC, last_access ASC LIMIT ?`,
   ...
   */
  async housekeeping({ dedupThreshold = 0.92, agingDays = 30, dryRun = true, autoMergeThreshold = 0.95, archiveEpAfterDays = 0, limit = 20 } = {}) {
    ...
    const details = []
    if (!dryRun) {
      // ① 几乎重复（≥ autoMergeThreshold）：择优保留一条，其余**归档**（不删除、不拼接）。
      //    v0.12.1 纠偏：旧实现把两条内容用 `\n---\n` 拼起来——越拼越长（库内实测最长 9099 字、
      //    同一个"早上好啊"被拼了三遍），那叫堆叠不叫整合。"合成一条更完整的结论"是 LLM 归并器
      //    （memory_consolidate）的活；这里只处理"本来就是同一件事"的近乎完全重复。
      const handled = new Set()
      for (const d of duplicates) {
        if (handled.has(d.a) || handled.has(d.b)) continue
        if (d.sim < autoMergeThreshold) continue
        const a = this.get(d.a)
        const b = this.get(d.b)
        if (!a || !b) continue
        // target：强度高者；同强度取信息更全（内容更长）者
        ...
        await this.mergeMemories(target.id, [source.id], {})
        handled.add(target.id)
        handled.add(source.id)
        merged++
        details.push({ target: target.id, archived: source.id, sim: d.sim, content: target.content.slice(0, 60) })
      }
      // ② 陈旧情景快照归档（ep 层 + 闲置超 N 天 + 强度从未升过）
      if (archiveEpAfterDays > 0) {
        const cands = this.epArchiveCandidates(archiveEpAfterDays)
        if (cands.length > 0) {
          archivedEp = this.archiveMemories(cands.map((c) => c.id), 'ep-aging')
          details.push({ epArchived: archivedEp, sample: cands.slice(0, 3).map((c) => c.content.slice(0, 40)) })
        }
      }
    }
    return { duplicates, aging, merged, archivedEp, details }
  }
  ...
  /** 阶段四（v0.9.5）：运行日志——透明可见（写入/注入/检索/巡检/蒸馏/错误）。
   *  惰性裁剪：每 64 条插入清理一次超出 maxRows 的旧日志；日志失败绝不影响主流程。 */
  log(level, event, detail = '', scope = '', maxRows = 2000) {
    try {
      const d = typeof detail === 'string' ? detail : JSON.stringify(detail)
      this.db.prepare('INSERT INTO logs (ts, level, event, scope, detail) VALUES (?, ?, ?, ?, ?)')
        .run(Date.now(), String(level), String(event), String(scope ?? ''), d.slice(0, 2000))
      this._logInsertCount = (this._logInsertCount ?? 0) + 1
      if (this._logInsertCount % 64 === 0) {
        this.db.prepare('DELETE FROM logs WHERE id NOT IN (SELECT id FROM logs ORDER BY id DESC LIMIT ?)').run(maxRows)
      }
    } catch { /* 日志写入失败不影响主流程 */ }
  }

  /** 阶段四（v0.9.5）：读取日志（时间倒序，可按级别/事件过滤）。 */
  listLogs({ limit = 100, level, event } = {}) {
    const where = []
    const args = []
    if (level) { where.push('level = ?'); args.push(level) }
    if (event) { where.push('event = ?'); args.push(event) }
    const sql = `SELECT * FROM logs${where.length > 0 ? ' WHERE ' + where.join(' AND ') : ''} ORDER BY id DESC LIMIT ?`
    return this.db.prepare(sql).all(...args, limit)
  }

  /**
   * 阶段四（v0.9.0）：事件检测——时间线扫描（纯 rule，无 LLM）。
   * sm 记忆按 created_at 排序；相邻两条「时间间隔 < gapMs」且「同 theme（非空）或共享实体（label 交集）」
   * → 归入同一事件；否则切新事件（单条记忆自成一事件，保证每记忆必属一事件）。
   * 全量重建 events/event_members；事件 id = ev-首成员记忆 id（重建稳定；
   * 约定：群首成员因时间排序变化时事件 id 会变，GUI 每次刷新快照即自愈，不做跨次持久引用）。
   * 返回 [{ id, label, startAt, endAt, count }]。
   */
  /** 事件检测：记忆 → 实体 label 集合（双源：node_memories 归一化节点 + nodes.memory_id 回退）。 */
  #buildEventLabels() {
  ...
  /** 时间线扫描聚簇：rows = [{id, theme, type, created_at}]（须按 created_at, id 有序）。
   *  时间连续（< gapMs）+ 因果相关（同主题 or 共享实体）→ 同一事件。 */
  #eventGroups(rows, nodeMems, gapMs) {
  ...
  /** 把聚簇组写入 events/event_members（事务内）。返回事件摘要数组。 */
  #eventGroupsToDb(groups) {
    const now = Date.now()
  ...
  /** 事件检测（**全量**）：扫描全部 sm 记忆重建事件表。供测试、detect=true 强制重建、迁移使用。 */
  detectEvents(gapMs = 2 * 3600 * 1000) {
    ...
    const nodeMems = this.#buildEventLabels()
    return this.#eventGroupsToDb(this.#eventGroups(rows, nodeMems, gapMs))
  }

  /**
   * 事件检测**增量**（避免每次启动/巡检全量重建）：水位线 meta['event_scan_at']。
   * 仅当存在 created_at > 水位线 的新 sm 记忆时，重建**尾部窗口**
   * （tailFrom = 最早新增时刻 - gapMs 起）；旧事件不动，开销 O(尾部+新增) 而非 O(全量)。
   * 无新增 → 直接返回 []（事件表保持现状）；跨重启幂等。
   */
  detectEventsIncremental(gapMs = 2 * 3600 * 1000) {
    const watermark = Number(this.getMeta('event_scan_at') ?? 0)
    const fresh = this.db.prepare(
      "SELECT id, theme, type, created_at FROM memories WHERE layer = 'sm' AND created_at > ? ORDER BY created_at, id",
    ).all(watermark)
    if (fresh.length === 0) return []
    const tailFrom = fresh[0].created_at - gapMs
    // 删除可能与新窗口重叠的尾部事件（只动尾巴，旧事件保留）
    const delTail = this.db.prepare('DELETE FROM events WHERE start_at >= ?')
    const delTailMems = this.db.prepare('DELETE FROM event_members WHERE event_id IN (SELECT id FROM events WHERE start_at >= ?)')
    this.db.exec('BEGIN IMMEDIATE')
    try {
      delTailMems.run(tailFrom)
      delTail.run(tailFrom)
      this.db.exec('COMMIT')
    } catch (err) {
      this.db.exec('ROLLBACK')
      throw err
    ...
    const nodeMems = this.#buildEventLabels()
    const out = this.#eventGroupsToDb(this.#eventGroups(tailRows, nodeMems, gapMs))
    this.setMeta('event_scan_at', String(fresh[fresh.length - 1].created_at))
    return out
  }
  ...
  events(limit = 20) {
    ...
    if (rows.length === 0) return []
    // 一次取回全部成员的 event_id → 内存分组
    const memberRows = this.db.prepare(
      `SELECT em.event_id, m.id, m.content, m.type, m.created_at
       FROM event_members em JOIN memories m ON m.id = em.memory_id
       WHERE em.event_id IN (${rows.map(() => '?').join(',')}) ORDER BY m.created_at`,
    ).all(...rows.map((e) => e.id))
    const byEvent = new Map()
    for (const r of memberRows) {
      if (!byEvent.has(r.event_id)) byEvent.set(r.event_id, [])
      byEvent.get(r.event_id).push({ id: r.id, type: r.type, content: r.content })
    }
    return rows.map((e) => {
   ...
   *  倒挂边（from 晚于 to）断开并重建正确方向（保留原权重）；同时间戳无法判定方向的保持不动。
   *  注意：手动建的逆语义 before 边（"受…影响"）会被自动反转，属约定行为。返回修正条数。 */
  fixBeforeDirections() {
    const rows = this.db.prepare(
      "SELECT from_memory, to_memory, weight FROM memory_links WHERE type = 'before' AND valid_to IS NULL",
    ).all()
    const getAt = this.db.prepare('SELECT created_at FROM memories WHERE id = ?')
    const hasLink = this.db.prepare(
      "SELECT 1 FROM memory_links WHERE type = 'before' AND valid_to IS NULL AND from_memory = ? AND to_memory = ?",
    )
    let fixed = 0
    this.db.exec('BEGIN IMMEDIATE')
    try {
      for (const r of rows) {
        ...
        // 倒挂：断开旧方向，重建正确方向（保留权重；正确方向已存在则仅断开）
        this.stmt.memoryLinkDeactivate.run(Date.now(), r.from_memory, r.to_memory, 'before')
        if (!hasLink.get(r.to_memory, r.from_memory)) {
          this.stmt.memoryLinkUpsert.run(r.to_memory, r.from_memory, 'before', r.weight, Date.now())
          fixed++
        }
      }
      this.db.exec('COMMIT')
  ...
  /** 向量路实况（v0.11.1 透明化）：`vector: true` 曾经掩盖了「嵌入已降级到 rule 兜底」的真相
   *  ——vecEnabled 只说明表能读，不说明向量是谁算的。这里把维度/规模/降级态一起报出来。 */
  vectorInfo() {
    let rows = 0
    try {
      rows = this.db.prepare('SELECT COUNT(*) AS c FROM memory_vectors_rowids').get().c
    } catch { /* 无 vec0 影子表（扩展未加载/从未写入） */ }
    return {
      enabled: this.vecEnabled,
      dim: this.vecDim,
      rows,
      embedder: this.embedder?.name ?? 'none',
      degraded: this.degraded,
    }
  }

  stats() {
    // v0.12.1：活跃与归档分开计数——归档不进检索，混在一起报会让人误以为库还在长
    const r = this.db.prepare(
      `SELECT COUNT(*) AS total,
              SUM(CASE WHEN archived = 0 THEN 1 ELSE 0 END) AS active,
              SUM(CASE WHEN layer = 'ep' AND archived = 0 THEN 1 ELSE 0 END) AS ep,
              SUM(CASE WHEN layer = 'sm' AND archived = 0 THEN 1 ELSE 0 END) AS sm,
    ...
    const vec = this.vectorInfo()
    return {
      memories: r.active ?? 0,          // 活跃记忆数（不含归档；归档见 archived 字段）
      ...
      // v0.11.1：向量路实况（谁算的向量、几维、多少条、是否降级），不再只有一个布尔
      vecDim: vec.dim,
      vecRows: vec.rows,
      embedder: vec.embedder,
      degraded: vec.degraded ?? false,
