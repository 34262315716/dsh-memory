  ...
  async search(query, { scope, limit = 8, minScore = 0, excludeIds = [], boost = {} } = {}) {
    ...
    if (boost && Object.keys(boost).length > 0 && scored.length > 0) {
      ...
      for (let i = 0; i < ids.length; i += 200) {
        ...
        const rows = this.db.prepare(`SELECT id, type FROM memories WHERE id IN (${chunk.map(() => '?').join(',')})`).all(...chunk)
        for (const r of rows) typeMap.set(r.id, r.type)
      }
      for (const h of scored) {
        const raw = boost[typeMap.get(h.id)]
