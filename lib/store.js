...
export class MemoryStore {
  ...
  detectEvents(gapMs = 2 * 3600 * 1000) {
    ...
    // 记忆间共享实体应比 label。生产运行中 node_memories 仅由 rebuild-graph.mjs 归一化时写入，
    // 常规写入路径只有 nodes.memory_id，故双源合并：node_memories JOIN + nodes.memory_id 回退）
  ...
  /** 阶段四（v0.9.0）：事件列表（含成员摘要，按时间倒序）。成员单条 JOIN 预取（无 N+1）。 */
  events(limit = 20) {
