...
export class MemoryStore {
  ...
  constructor(dbFile, opts = {}) {
    ...
    try {
      ...
      console.warn('[dsh-memory] theme 列检测失败: ' + err.message)
    }
  ...
  async themeMemories(threshold = 0.78) {
    ...
    try {
      for (const cl of clusters) {
        ...
        const label = [...wc.entries()].sort((a, b) => b[1] - a[1]).slice(0, 2).map(([w]) => w).join('/')
  ...
  /** 新增一条记忆。返回 id。 */
  async add({ layer = 'sm', type = 'note', scope = 'global', content, keywords = [], strength = 1 }) {
  ...
  /** 阶段三：记忆级连边（8 型）——两记忆的节点集跨集连 type 边；幂等（同三元组去重）。返回新建边数。 */
  link(fromMemoryId, toMemoryId, type, weight = 1) {
    if (!EDGE_TYPES.has(type)) throw new Error(`unknown edge type: ${type}`)
    const fromNodes = this.db.prepare('SELECT id FROM nodes WHERE memory_id = ?').all(fromMemoryId)
    const toNodes = this.db.prepare('SELECT id FROM nodes WHERE memory_id = ?').all(toMemoryId)
    if (fromNodes.length === 0 || toNodes.length === 0) return 0
    const now = Date.now()
    this.db.exec('BEGIN IMMEDIATE')
    let n = 0
    try {
      for (const f of fromNodes) {
        for (const t of toNodes) {
          ...
          this.stmt.insertEdge.run(edgeIdFor(f.id, t.id, type), type, f.id, t.id, now, weight)
          n++
        }
      }
      this.db.exec('COMMIT')
  ...
  /** 阶段三：断开记忆间 type 边（valid_to 置为 now，历史保留不销毁）。返回断开条数。 */
  unlink(fromMemoryId, toMemoryId, type) {
    ...
    try {
      for (const f of fromNodes) {
        for (const t of toNodes) {
          n += this.db.prepare(
            'UPDATE edges SET valid_to = ? WHERE from_node = ? AND to_node = ? AND type = ? AND valid_to IS NULL',
          ).run(now, f.id, t.id, type).changes
        }
      }
      this.db.exec('COMMIT')
      return n
  ...
  /** 阶段三：记忆级单边——两记忆的代表节点间连一条 type 边（避免节点集全连接爆炸）。 */
  linkMemories(fromMemoryId, toMemoryId, type, weight = 1) {
    if (!EDGE_TYPES.has(type)) throw new Error(`unknown edge type: ${type}`)
    const f = this.nodesOfMemory(fromMemoryId)[0]
    const t = this.nodesOfMemory(toMemoryId)[0]
    if (!f || !t || f.id === t.id) return 0
    this.stmt.insertEdge.run(edgeIdFor(f.id, t.id, type), type, f.id, t.id, Date.now(), weight)
    return 1
  }
