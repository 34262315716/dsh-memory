...
export function registerGraphTools(ctx, store, getCfg) {
  ...
  safeRegister(defineTool({
    ...
    description: '查询一条记忆的图谱邻域（k-hop 扩散，记忆级 v0.9.33）：沿 memory_links 活跃语义边扩散，返回相邻记忆 id、边类型与跳数。',
    parameters: {
    ...
    output: {
      ...
      render: (_args, value) => {
        if (value.nodes.length === 0) return [{ type: 'text', text: '该记忆没有相关联的记忆（图谱开关开启且写入后才有）。' }]
        const lines = value.nodes.map((n) => `  [${n.depth}跳 · ${n.type}] ${n.id} ${n.snippet}`)
        return [{ type: 'text', text: `记忆图谱邻域（${value.nodes.length} 条记忆）：\n${lines.join('\n')}` }]
      },
    },
    execute(args) {
      const nbrs = store.memoryLinkNeighbors(args.memoryId, args.hops ?? 2)
      return {
        nodes: nbrs.map((n) => {
          const m = store.get(n.id)
          return { id: n.id, type: n.type, depth: n.depth, snippet: truncate(m?.content ?? '', 50) }
        }),
      }
    },
  }))
  ...
  safeRegister(defineTool({
    ...
    description: '查询两条记忆之间的图谱最短路径（记忆级 v0.9.33）：沿 memory_links 活跃语义边 BFS，返回记忆 id 序列与边类型链。起点/终点用记忆 id（如 mem-xxxx）。',
    parameters: {
      fromId: { type: 'string', required: true, description: '起始记忆 id（如 mem-xxxx）' },
      toId: { type: 'string', required: true, description: '目标记忆 id' },
    ...
    output: {
      ...
      render: (_args, value) => {
        if (!value.found) return [{ type: 'text', text: '两条记忆之间没有路径。' }]
        const steps = value.nodes.map((n, i) => (i === 0 ? n : ' --' + value.edges[i - 1] + '--> ' + n))
        return [{ type: 'text', text: '记忆最短路径：\n' + steps.join('\n') }]
      },
    },
    execute(args) {
      const p = store.memoryPath(args.fromId, args.toId, args.maxLen ?? 6)
