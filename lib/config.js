...
export const Config = z.object({
  ...
  /** 步距节流：每 N 步全量重检索（步距到必检，同 query 也重检；重复注入由内容 hash 去抖） */
  stepInterval: z.number().min(1).max(10).default(10),
  /** 每个 agent 最近注入窗口（防循环） */
  ...
  refiner: z.object({
    ...
    /**
     * 推理档位（随 provider route 透传）：'off' = 关闭思维链，纯文本直出。
     * 必须显式关闭——推理模型默认档位会把 maxTokens 全耗在 reasoning_content 上，
     * 导致 JSON 正文为空/截断（v0.9.25 修复：refiner 曾因此 100% 降级）。
     * 仅适配器声明 supportsReasoningEffort 时生效；off → 内核映射为关闭推理。
     */
    reasoningEffort: z.string().default('off'),
    /** 输出最大 token（蒸馏 JSON 较长，800 在推理关闭后亦偏紧）。 */
  ...
  graphView: z.object({
    ...
    /** 布局质量（v0.10.13）：precise=算到"所有点都不动"再展示（默认，预算 4s / 8000 步）；
     *  balanced=限时 1.2s / 3000 步；instant=不重算，直接用缓存或初始布局。
     *  节点数超阈值（900/1800）会按此顺序自动降档。非法值回落 precise。 */
    layoutQuality: z.string().default('precise'),
