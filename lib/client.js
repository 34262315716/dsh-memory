...
function MemorySettingsSectionInner({ scope, api, llmScope, deepseekScope }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (gvKeys.some((f) => drafts[`graphView.${f}`] !== void 0) || drafts["graphView.themeShape"] !== void 0) {
...
var ObsidianGraph = (0, import_react2.memo)(function ObsidianGraph2({ data, onSelect, selectedRef, drawRef, physics, focusIdsRef }) {
  ...
  (0, import_react2.useEffect)(() => {
    ...
    const draw = () => {
      ...
      const themeGroups = /* @__PURE__ */ new Map();
      for (const n of nodes) {
        const t = n.theme;
        if (!t || t === "(\u672A\u5F52\u7C7B)") continue;
      ...
      for (const [theme, members] of themeGroups) {
        ...
        ctx.globalAlpha = (focusId ? members.some((n) => n.id === focusId) ? 0.9 : 0.18 : 0.9) * overallT * (dimmed ? 0.08 : 1);
        ctx.beginPath();
        if (bounds.kind === "hull") {
        ...
        ctx.setLineDash([]);
        ctx.stroke();
        ctx.globalAlpha = (focusId ? members.some((n) => n.id === focusId) ? 0.95 : 0.22 : 0.95) * overallT * (dimmed ? 0.08 : 1);
        ctx.font = `bold ${11 / transform.k}px sans-serif`;
        ctx.fillStyle = `hsla(${hue},70%,72%,0.9)`;
        ctx.textAlign = "center";
        ctx.fillText(members.length + " \xB7 " + theme, bounds.cx, bounds.topY - 8 / transform.k);
      }
      ctx.setLineDash([]);
