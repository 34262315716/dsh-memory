...
| **LLM 蒸馏（refiner）** | 独立模型把高噪声轮次提取为自包含结论（决策/偏好/教训分类）+ **双输出（v0.10.0）**：`abstract` 抽象层级（principle=可复用原则/方法 / event=一次性事件产出）+ `theme` 名词标签（如"四级备考"）；失败自动降级规则路径 |
...
client/graph-geometry.js 主题圈几何（贴合凸包 / 最小外接圆；纯函数，与守护测试共用）
...
node test-graph-geometry.mjs # 主题圈几何专项（31 项：凸包贴合/外扩精度/最小外接圆最优性与确定性/随中心平移跟随；纯函数，无需副本环境）
