...
console.log('== 8. layout-policy：布局质量档位与自动降档（v0.10.13） ============')
{
  ...
  const big = resolveLayoutPlan({ quality: 'precise', nodeCount: 2500 })
  check('大图（>2000）precise 自动降到 balanced + 标记', big.mode === 'balanced' && big.downgraded && big.reason.includes('2000'))
  const huge = resolveLayoutPlan({ quality: 'precise', nodeCount: 7000, hasCache: true })
  check('超大图（>6000）+ 有缓存 → instant（直接用缓存）', huge.mode === 'instant' && !huge.compute && huge.downgraded)
  const hugeNoCache = resolveLayoutPlan({ quality: 'balanced', nodeCount: 7000, hasCache: false })
