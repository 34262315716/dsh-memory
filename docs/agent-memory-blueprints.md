...
> **决策（2026-08-18）**："何时注入/注入什么记忆"由**软件工程**（显式代码规则）实现；训练驱动的"记忆方法论/记忆技能"路线（MemSkill/meta-memory，需 RL 训 controller）**不纳入**——兄弟：训不了模型，且 LLM 无法从少数样本习得"何时记/何时忘"的方法性知识。注入侧规则（stepInterval/minScore/防循环窗/KV 缓存友好/scope）是特性。
