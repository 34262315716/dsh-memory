...
- **主题圈** ✅ **已完成（v0.10.1，v0.10.5 重做）**：同主题节点（成员 ≥3、非"未归类"）绘制半透明淡色包围区 + 主题名标签（`成员数 · 主题`），随聚焦降透明。**v0.10.5 修正**：初版是「质心 + 最远点 + 固定 26px」粗圆，弧状/条状分布会圈进大片空地 → 按成员实时坐标逐帧计算贴合形状。**v0.10.6 再修正**：主题是语义分组、空间上常摊得很开，对全体成员取全局凸包会兜住别家节点（实拍糊满画布）→ 改为**只圈密集团**（就地聚簇 + 密度核心，滤稀疏末端）、区域画到边与节点之下、显示策略可配（`themeScope`：focus 悬停/选中时显示＝默认 / always 常显密集团 / off 关闭）。设置面板「检索与注入」已补 v0.10 维度说明（P1 GUI 遗留一并结清）；「记忆图谱」区块新增主题圈形状下拉。
