...
- **主题圈** ✅ **已完成（v0.10.1，v0.10.5 重做）**：同主题节点（成员 ≥3、非"未归类"）绘制半透明淡色包围区 + 主题名标签（`成员数 · 主题`），随聚焦降透明。**v0.10.5 修正**：初版是「质心 + 最远点 + 固定 26px」粗圆，弧状/条状分布会圈进大片空地 → 改为按成员实时坐标逐帧计算**贴合凸包**（默认可切换最小外接圆），padding 跟随节点视觉半径，圈随主题中心与组形实时移动。设置面板「检索与注入」已补 v0.10 维度说明（P1 GUI 遗留一并结清）；「记忆图谱」区块新增主题圈形状下拉。
