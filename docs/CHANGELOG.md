...
- **工具：一个都不传。** `lib/refiner.js` 的 `streamCollect()` 显式 `tools: []`（v0.12.5 的根因修复）——带上工具时模型会改走 `tool_calls`、正文 0 字直接降级。
- **提示词**：系统提示只有一句（`你是严格的 JSON 输出器，只输出合法 JSON。`）；用户消息 = 角色句 + `EXTRACT_RULES`（约 1180 字）+ 库里已有记忆（每条 220 字、**无条数上限**）+ 格式说明 + 对话正文（每侧最多 6000 字）→ 单次输入约 13.5k 字。而线上 4 次降级的思考是 14122 / 34052 / 45327 字 = **输入量的 1~3.4 倍**。

### 二、三个真凶（证据：memory_logs 里那 4 次降级）

1. **提示词在命令模型「认真写长」**：`【第一步：analysis（你的判断过程，会被审计，请认真写，不要敷衍）】用 2-5 句回答…`。对**强制思考**的 deepseek-v4.1-flash，这是火上浇油——先在思考里把判断论证一遍，再在正文里写一遍。
2. **续跑判据写错了（最贵的一条）**：`truncatedLike()` 用 `acc.includes('{')`——正文里**出现过**一个 `{` 就算「JSON 写了一半」。而实测形态恰恰是**模型把判断写成了散文**（正文 1854 字、`finish=stop`），散文里顺手夹了个 `{` → 3 次续跑全在催「接着写」，每次都要重新思考一遍，白烧 3 次调用且照样救不回来。
3. **输入给得太大**：6000+6000 字对话 + 无上限的旧记忆清单——读得越多、想得越多。

另有一条口径问题：`meta.reasoningChars` 只是**当前缓冲**长度，跨调用的思考被合起来看成一个数，正是这次先误判成「一次想 4 万字」的原因。

### 三、改法

- **提示词瘦身**：`analysis` 改为「1-2 句直给结论，不要展开论证、不要复述对话、不要自检规则」，删掉「会被审计，请认真写，不要敷衍」；items 要求 6 条并成 5 条；keywords 的四组禁止词收成三行；开头补一句「**不要在思考里复述对话、也不要逐条复读下面的规则**」。
- **续跑判据改成「正文已经在写 JSON」**（`looksLikeJson()`：以 `{`/`[` 开头，或已出现 `"analysis"/"items"/"content"/"keywords"` 字段名）。散文骗不到续跑了，改走一次针对性强化重试；重试时**先清空缓冲**（`call(..., { reset: true })`）——不清的话，散文里那个 `{` 会把重试吐出的合法 JSON 一起毒死（`parseJsonLoose` 只从**第一个** `{` 取到最后一个 `}`）。这个洞是被新套件当场抓住的，老代码同样存在，只是旧判据走不到那条路。
- **输入瘦身**：新增 `refiner.dialogChars`（默认 4000，可调 500~20000）、`refiner.knownChars`（默认 120）、`refiner.knownTopN`（默认 5，此前**无上限**）。
- **诊断能回答「思考为什么长」**：`meta.calls` 逐次记账（每次的正文/思考字数、finish、耗时），并新增跨调用累计的 `meta.reasoningCharsTotal` / `textCharsTotal`；诊断行现在写 `思考 N 字（跨调用累计，逐次见下）；逐次调用：[#1 …，#2 …]`。

### 四、效果与测试

- 最坏调用数：散文跑题从 **5 次**（1 首调 + 3 续跑 + 1 重试）降到 **2 次**；单次输入从约 13.5k 字降到约 9k 字。
- 新增套件 `test-refiner-slim.mjs`（30 项）：散文带 `{` 不再触发续跑、空正文/截断仍照旧续跑、逐次记账与累计值、提示词确实变短且要求没被删掉、对话与旧记忆的截断与上限生效。
- 全量回归：24 套件 / 649 项 / 0 实质失败（唯一失败项是 `test-embedder` 的「真实 API」——排查确认**本机代理未运行**（7890 无监听进程），属环境问题，非回归）。

## v0.13.2 — 提取探针不再把冷启动抖动当成「模型不可用」（2026-09-26）

重启后第一次调 `/dsh-memory/health?llm=1` 报 `ok:false`：A 组（`tools: []`，与真实提取同构的那条）只回到 `[usage,finish]` 的空回包。但同一进程紧接着再探两次，A 都成功（8.6s / 10.3s，`探测A（tools=[]）成功`）—— **空回包 ≠ 提取模型坏**，是冷启动/上游抖动。

这正是教训 `mem-1b4267ae` 说的「仪器骗人」：探针把瞬时故障说成稳定结论，会让人跑去修一个没坏的东西（本机为此已经白查过一次）。

改法：A 空手而归时（且不是预算到点）自动重试一次，只有连续两次都空才落进 B/C 定位；重试成功时 note 里如实写明「首次空回包、重试成功」，不掩盖抖动。另外健康检查同时接受 `?llm=1` 与 `?withLlm=1`（前者是实现里的名字、后者是面板/文档里的写法，两个都认，免得再被参数名误导）。
...
### 一、同构探针 A/B（真实 provider / 模型 / prompt 结构 / `x-opencode-session` 头）

| 变体 | finish_reason | completion | reasoning | 正文 | JSON |
|---|---|---|---|---|---|
| `max_tokens=800`（当时 settings） | **`length`** | 800 | **800（吃光）** | **0 字** | ❌ 空 |
| 不传上限（v0.12 设计默认） | `stop` | 2784 | 1795 | **2030 字** | ✅ 直接可解析 |

**结论**：`deepseek-v4.1-flash` 是**强制思考**模型，**思考 token 也计入 `max_tokens`**。800 被思考全部吃掉 → 上游按长度截断 → 正文为空 → `JSON.parse('')` → 降级。输入越长思考越多，于是"长输入才失败"完全对得上。

### 二、根因：设计已经改对，配置把它埋了回去

v0.12 的设计注释写得很清楚——"`maxTokens: 0` = 不限制……'预算不够导致正文为空'这一类故障从根上消失"。但 `settings.yaml` 里残留着 `maxTokens: 800`，把设计里已经消掉的故障又请了回来。**这是典型的"代码对了、配置错了"，而两者都在仓库之外。**

### 三、修复（诊断 + 解析 + 护栏，不动成功路径与重试语义）

1. **设置**：`memory.refiner.maxTokens` → `0`（附注释说明复现数据）。设置文件由 `dsh-settings-file` 的 chokidar 监听热重载，**改完即时生效**。
2. **诊断可读化**：新增 `fmtFinish()` —— `finish.reason` 是**对象** `{kind,failure?}`，以前直接插值成 `[object Object]`，把最关键的终止原因自己吞掉了；现在打印 `kind｜failure.message｜code`，并在 `kind='max-tokens'` 时直接给出"把 maxTokens 设为 0"的建议与当前配置值。
3. **老 bug**：`if (r.finish === 'length')` —— **拿对象比字符串，恒为 false**，于是 `meta.truncated` 从来没被置过位、截断识别形同虚设。改为 `r.finish?.kind === 'max-tokens'`（`kind` 的正确取值见 `dsh-llm-pi-ai` 的 `mapStopReason`）。
4. **截断 JSON 修复**：新增 `repairTruncatedJson()` —— 退到"最后一个完整容器闭合处"补全括号，**保住已经写完的 analysis 与条目**。截断从此不等于降级（实测：截断在条目之间的输出能救回已完成的条目）。
5. **误配置护栏**：启动时若 `refiner.maxTokens > 0` 就打 warn，别让它再静默复发。
6. **探针预算 25s → 60s**：实测一次正常提取要 20.3 秒，25 秒的探针**会在真实路径正常时报失败**——再一次"探针与被检验路径不同构 → 仪器骗人"。

### 四、测试

新增 `test-refiner-truncation.mjs`（28 项）：finish 可读化 / 截断修复 / 修不了时诊断指名道姓 / 截断可修复时不降级 / 续跑语义回归 / 正常输出不受影响。
全量 **24 个套件 623 项通过，0 失败**。

### 五、教训

- **诊断信息不能是黑盒**：`[object Object]` 让一次排查绕远——最关键的终止原因就在手边，却被格式化吃掉。
- **"对象比字符串"是静默 bug**：不抛错、不报警，只是永远为 false；本仓库第 6 次同源事故。
- **代码对了不等于系统对了**：配置、探针、诊断任一环节撒谎，故障都会以别的面目出现。

## v0.12.7 — 降级记忆「由 Agent 撰写」的改写通道 + 242 条存量迁移（2026-09-23）

v0.12.6 修好了提取链路，但停机 7 天里写进库的降级痕迹（`任务: … 结果: …`）还在。`memory_rewrite` 是让**插件自己的提取器**逐条重写；本次补上它的姊妹通道——**由外部（人 / 主 Agent / 子代理）写好正文**，插件只负责落库。

### 一、新工具 `tools/apply-rewrite.mjs`

读一份计划文件（`{items:[{id, action, content, theme, abstract, keywords, layer, type}]}`），逐条应用：

| 动作 | 行为 |
|---|---|
| `rewrite` | `store.update()` 换正文 —— **世界线版本 + FTS 由插件自己的语句处理**，不手抄 SQL |
| `archive` | `store.archiveMemories()` —— 归档≠删除，`memory_archive` 可恢复 |
| `skip` | 不动（缺省即此，宁可漏不误伤） |

安全：默认只预演；`--apply` 才写；写前 `VACUUM INTO` 一致性快照；每条写后回读校验。

### 二、踩到的坑：假向量（指标会骗人）

脚本没有嵌入凭据，`store.embedTexts()` 在无 embedder 时会走 `ruleEmbed(t, vecDim)` —— **它按向量表维度造了一个兜底向量并成功写入**。于是 `vecRows` 看起来是齐的，实际那条是**不在远程嵌入空间里的垃圾向量**；更麻烦的是 `reembedMissing` 只补「行不存在」的，**永远不会修它**（`vecRows == 总行数` 这个指标会骗人）。

对策：写正文后**主动删掉向量行**（脚本内 `DELETE FROM memory_vectors`；store 构造时已 loadExtension），收尾统一跑 `memory_reembed` 用真嵌入重算。实测一次补齐 185 条。

### 三、执行：25 批 × 10 条

- 把 242 条降级记忆按 10 条一批导出 → **5 个子代理**按 `RUBRIC.md` 并行判断 → 产出 `plan-NN.json` → 抽查质量后批量落库
- 判据（写进 RUBRIC）：**要留**的是决策与理由、结论与数据、环境/项目事实、教训、偏好画像、可复用方法；**归档**的是纯执行痕迹、临时状态、无结论探索、任务书/prompt 原文；**拿不准就 skip**
- 结果：**改写 187 · 归档 56 · 跳过 1**；活跃降级记忆 **242 → 1**（剩下的那条是用户个人处境自述，留人工复核）
- 分层随改写迁移：`sm 703 → 822`（不少降级痕迹本身就是稳定知识，只是当年没被提炼出来）

### 四、顺带的两件事

1. **密钥脱敏**：扫描发现 2 处 firecrawl 密钥残留（一条在正文，一条只在世界线历史里）。已在正文与历史中一并打成 `fc-***REDACTED***`——**历史版本不删段，只替换密钥片段**。其余 `sk-` 命中全是 `task-board` / `mask-image` / `disk-backed` 的误报。
2. **降级路径仍在触发**：迁移期间又新写入 3 条降级记忆（11:07 / 14:21 / 14:22 的长 prompt 与长回答、以及子代理任务书原文）。即「模型对超长输入吐不出合法 JSON」那条问题**仍在发生**，不是历史遗留。

## v0.12.6 — 根因落定：缺一个 sessionId，瘫痪了 7 天（2026-09-23）

用户一句话点醒了我：「**不要乱改，你不如把所有的代码都看一遍**」。停止东改西改、改成把调用链完整读一遍之后，根因半小时内浮出水面。

### 一、根因（每一环都有代码原文）

| # | 环节 | 代码依据 |
|---|---|---|
| 1 | 插件调 `ctx.llm.stream(...)` 时**没传 `sessionId`** | 主对话传的是 `sessionId: this.session.id` |
| 2 | 适配器因此**不加** `x-opencode-session` 头 | `dsh-llm-pi-ai`：`const sid = sessionId === void 0 ? void 0 : String(sessionId); if (sid === void 0) return headers;` |
| 3 | 上游拒收 | 该函数注释原文：「*required by opencode.go from 09/05; missing it yields HTTP 400 MissingSessionID*」 |
| 4 | pi-ai **不抛错**，把失败变成 error 事件 | 官方注释：「*pi-ai never throws mid-stream — failures arrive as error events*」 |
| 5 | error 被转成 **usage + finish** 两块后正常结束 | `toStreamChunks` 的 `case "error":` 分支 |
| 6 | 插件只收 text-delta/reasoning-delta → **"模型返回空正文"** | —— |

**这解释了全部现象**：直连正常（手动加了头）、换 provider 无效（阿里百炼同样走这条判定）、tools/请求头/输入规模全都无关、finish 存在却没内容。

### 二、为什么查了整整一晚

`finish` 块的 `reason` 里**一直带着完整错误信息**：

```js
{ kind: "error", failure: { message: "model … returned a completed response with no content", code: EMPTY_RESPONSE_CODE } }
```

**代码存了它，却从来没有读过它。** 所有"猜 provider、猜 header、猜 tools"的工夫，都源于这行信息没被打印出来。

### 三、修复

1. **所有 LLM 调用补传 `sessionId`**（逐轮提取传 `session.id`；会话汇总、健康探针同样）——**根治**。
2. **显式识别 `finish.reason.kind === 'error'` 并抛出**（带 `failure.message` 与 `code`）——以后任何静默失败都会直接报出原因。注意 `aborted`（时间预算掐断）是**另一个 kind**，不影响续跑逻辑，这条专门写了测试钉住。

### 四、新增：降级记忆重写工具 `memory_rewrite`

提取瘫痪期间写进库的 **309 条**「任务: X 结果: Y」降级产物，需要的不是清理而是**救回来**：

- 逐条拆回"用户/助手"两侧 → 交回**已修好的提取器**重新判断 → 产出正常自包含结论 → **更新原条目**（旧文进世界线，可回滚）
- 提取器判定"无长期价值"的 → 归档（不是删除）
- **天然可续跑**：处理过的条目正文不再以「任务:」开头，下次扫描自动跳过
- 默认 `dryRun: true` 只预演；分批调用即可全量

### 五、施工中自己闯的一次祸（如实记录）

用脚本改记忆时，我用 **rule 兜底嵌入器（256 维）**打开了生产库，且**没传 `degraded` 护栏**，触发 store 的「向量维度迁移」——**4096 维向量表被 DROP 重建成了 256 维空表**。

- **记忆内容一条没丢**（memories 表完好）
- 用正确配置（remote / Qwen3-Embedding-8B / 4096）重开一次即自动重建回 4096 维，随后 `memory_reembed` 把 **1292 条全部补齐**（缺 0）
- **根因**：`lib/index.js` 里本来有这道护栏（`degraded: embedder.name === 'rule' && (embCfg.provider ?? 'rule') !== 'rule'`），是脚本**绕过了它**。教训：**任何直连生产库的脚本都必须复用插件的构造路径**，不能自己拼参数。

### 六、验证

- 新增 `test-rewrite.mjs`（**15 项**：拆分 / 候选筛选含"防空数组假通过" / 重写走更新且旧文进世界线）
- `test-refiner-thinking.mjs` 60 → **66 项**（sessionId 透传、不污染其他 provider、error 必须抛、aborted 不算硬失败）
- **全量 21 套 601 项全绿**；部署 md5 复验一致；gitnexus 索引重建（2616 节点 / 9369 边 / 159 流程）

## v0.12.5 — 根因落定：模型改用工具调用，所以正文永远是空的（2026-09-23）

用户重启后跑了一次自检，把最关键的一行带回来了：

```
提取 不可用 · opencode-go / deepseek-v4.1-flash
提取模型返回空正文（思考 0 字、正文回包 0 段、收到的块类型=[usage,finish]）
```

**块类型只有 usage 和 finish，一个 delta 都没有。** 这一行直接把范围从"通道坏了"缩到"上游确实没吐内容"。

### 一、排除法：把嫌疑逐个打掉

| 嫌疑 | 实验 | 结果 |
|---|---|---|
| 请求头（x-opencode-session / accept / user-agent） | 逐项复刻 adapter 的头 + 长 UUID / 32 位 hex 会话 id | ❌ 全部正常（正文 233–416 字） |
| max_tokens 被目录声明值污染 | 直连显式传 384000 | ❌ 正常 |
| 输入规模 | 直连长输入（2046+3166 字） | ❌ 正常（正文 750 字） |
| 请求体构造 | 直接 import adapter 自己的 buildChatBody 构造并发出 | ❌ 正常（正文 265 字） |

### 二、真凶：**`tools`**

用 adapter 自己的构造函数做最后一组对照，唯一变量是"请求里有没有工具清单"：

| 组 | 请求带 tools | 正文 | finish |
|---|---|---|---|
| A | ❌ | **265 字** | stop |
| B | ✅ | **0 字** | **tool_calls** |
| C | ✅ | **0 字** | **tool_calls** |

**模型看到有工具可用，就改用"调用工具"而不是输出正文。** DSH 调用 adapter 时会带上会话的工具清单，而提取任务会把这批工具原样透传下去——于是每一次提取都得到 `finish=tool_calls` + 空正文。

**这也解释了时间线**：9-16 之前用的是非推理模型，对 tool-calling 不敏感；换成 `deepseek-v4.1-flash` 后立刻复发——不是"预算"问题，是"**模型太听话了**"。

### 三、修法

- `lib/refiner.js` 的 `streamCollect()` 显式传 **`tools: []`**（adapter 只在 `tools.length > 0` 时才下发工具表，空数组即关闭）。
- health 探针同样传 `tools: []` —— 探针与被检验的真实路径保持同构（这条教训在 v0.12.4 刚吃过）。
- 提取、归并、会话汇总、画像蒸馏、主题重命名共用同一个 `llmStrictJson`，因此**一起修好**。

### 四、验证

- `test-refiner-thinking.mjs` 58 → **60 项**：新增"提取调用必须传 `tools: []`"与"`extractWithLlm` 同样清空工具表"两条防回归断言。
- **全量 20 套 580 项全绿**；部署并 md5 复验一致。

### 五、复盘：这个 bug 为什么藏了 7 天

1. **错误信息骗人**：从 "Unexpected end of JSON input" 到 "LLM 输出为空"，两条都指向"输出有问题"，没人会想到"模型去调工具了"。
2. **直连复现永远成功**：直连时自己不带 tools，所以怎么测都是好的——**被测路径与真实路径不同构**（和 v0.12.4 记下的教训同源）。
3. **没有记录 finish_reason 与块类型**：`finish=tool_calls` 这条线索一直存在，只是从来没被记下来过。补上诊断之后，一次自检就断案了。

## v0.12.4 — 提取仍在失败：定位到「通道层」，并加上兜底与诊断（2026-09-23）

起因：用户说「你再找找有没有什么其他的问题了」。一查日志，**提取其实还在失败**。

### 一、事实：v0.12.0 没治好它，只是换了错误信息

- 「write.refined」最后一次成功是 **9-16**；此后「write.fallback」累计：旧错误「Unexpected end of JSON input」**278 次** + v0.12.0 引入的新错误「LLM 输出为空」**15 次**。
- 也就是说：把"预算被思考吃光"这条路堵上之后，**又露出了另一条路**——模型压根没吐正文。

### 二、对照实验：问题不在模型，在 DSH 的通道

| 通道 | 输入 | 结果 |
|---|---|---|
| 直连 opencode-go（chat/completions） | 短输入 | ✅ 思考 915 字 + 正文 583 字 |
| 直连 opencode-go | **长输入（2046+3166 字）** | ✅ 思考 3155 字 + 正文 750 字 |
| 直连 + 显式 max_tokens=384000 | 短输入 | ✅ 正常（否掉了"目录声明的 384000 被当默认发出去"这个假设） |
| **DSH 的 ctx.llm.stream（health 探针）** | 同构请求 | ❌ **1.66 秒返回，思考 0 字、正文回包 0 段** |

结论：模型、输入规模、token 预算**都不是原因**；**DSH 那条通道送回来的东西是空的**。顺着 dsh-opencode-go 的 adapter 看了一遍（路由分支、session header、chat body 构造、SSE 解析）都没发现异常，所以下一步不再靠猜。

### 三、两手准备：兜底 + 诊断

- **兜底**：思考型模型有时把**结论整段写在思考里**、正文一个字不吐。思考流本身也是文本，里面常已包含完整 JSON——现在会**先试着从思考里捞一次**（meta.salvaged=true 标记，可审计），比直接判失败强。
- **诊断**：所有失败路径都带足信息——正文回包 N 段 / 思考 N 字 / 其他块 N 个 / **收到的块类型=[...]** / 末次 finish。以前只有一句"LLM 输出为空"，现在能一眼区分：
  - **块类型为空** → 适配器没送来任何 delta（通道问题）
  - **有 block-start/finish 却没 delta** → 上游确实没吐内容（模型/请求问题）
- health 探针同样带上这组诊断（它是最快的验证入口，不用等真实对话触发）。

### 四、顺手做的体检（按用户要求用 gitnexus）

- **索引重建**：此前落后 14 个提交，重建后 **2578 节点 / 9250 边 / 126 簇 / 154 流程**（旧索引 1712 节点，严重失真）。
- check：**无循环依赖**（clean）。
- impact 复核本批改动面：epArchiveCandidates 只有 1 个直接调用者（housekeeping，LOW）；stats 有 4 个调用点，其中 **graph-snapshot.js 只是把 store.stats() 原样透传**、没有二次计算，所以 v0.12.1 把 memories 语义改成"活跃数"是安全的。

### 五、验证

- 新增/扩充测试：test-refiner-thinking.mjs 50 → **58 项**（含"从思考流捞回 JSON 且只调用一次""思考里没 JSON 时不误捞""失败诊断必须含段数/字数/块数"）。
- **全量 20 套 578 项全绿**；部署并 md5 复验一致。

### 六、需要做的

重启 DSH 后**跑一次设置面板的「连通性自检」**——它会直接打印"收到的块类型"。有了这一行，通道层的问题就能一次定位。

## v0.12.3 — 存量清理：补连 + 归档，以及一路踩到的三个真 bug（2026-09-23）

起因：用户说「帮我把它们做完吧」——指存量那两件小事。动手前先备份，动完发现**三个真 bug**。

### 一、存量清理实况

- **语义补连**：`tools/semantic-links.mjs --apply` 写入 **75 条边**（活跃边 445 → **520**），其中 **16 条把"双方原本都孤立"的记忆接回了主图。一致性快照 + 回滚清单齐全（`--rollback <stamp>`）。
- **陈旧情景快照归档**：`tools/cleanup-ep.mjs --apply` 归档 **209 条**（7 天前 + 「任务:」形态 + 强度未被加固）。库：总计 1274 → **活跃 1065 / 归档 209**（活跃 ep 393、活跃 sm 672）。
- 归档后仍完全孤岛的活跃 sm 从 303 降到 **248** —— 剩下的不是"漏了"，是**真的没有足够相似的邻居**（四道闸门：τ 0.8 + 互为最近邻 + 每条上限 2 + 跨 scope 0.88）。这比硬连边更诚实。

### 二、路上踩到的三个 bug（都修了）

1. **`epArchiveCandidates` 判据写错，导致"陈旧 ep"永远是 0 条**（本批最要命的一个）。
   判据写的是 `last_access = 0`，而 `store.add()` 建库时就把 `last_access` 设成了创建时间 —— **该条件对新建记忆恒不成立**。实测 601 条 ep 里 0 条入选，其中 503 条还是"任务/结果"式降级产物。
   正解：`last_access <= created_at`（被检索命中过的会被 touch，`last_access` 才大于创建时间）。
2. **`tools/semantic-links.mjs` 遇到孤儿向量直接崩**：`memory_vectors` 里残留了 1 条"记忆已删但向量还在"的行，`byRow.get(b).id` 抛 TypeError。现在跳过孤儿并打印条数（顺带解释了 `stats()` 里 `vecRows` 比记忆数多 1 的由来）。
3. **改了 SQL 判据却没同步参数个数**：判据从 3 个占位符变 2 个，调用仍传 3 个 → `column index out of range`。测试当场抓到（`test-governance` 第 5 节）。

### 三、新增运维脚本

- `tools/cleanup-ep.mjs`：情景快照归档（**默认只预演**，`--apply` 才写库）。写库前 `VACUUM INTO` 一致性快照；只 `UPDATE archived=1`、**不删除**；打印各档判据下的候选量（1/3/7/14/30 天）与最早/最新样本，便于核对保留线。支持 `--all-shapes`（不限形态）与 `--require-unused`（要求从未被检索命中）。

### 四、验证

- **全量 20 套 570 项全绿**；`tools/deploy.mjs` 同步并 md5 复验一致。
- 两次写库都有独立快照（`memory.db.snapshot-*` / `memory.db.snapshot-epclean-*`），归档可用 `memory_archive` 恢复、补连可用 `--rollback` 回退。

## v0.12.2 — 对话流程：会话级汇总 + 纠正既有记忆（2026-09-21）

起因：用户第三件事「**提取日常对话中的内容也需要进行方法流程上的优化**」。查下去是两个缺口——不是"提取质量"问题，是**流程少了两层**。

### 一、缺口一：只有碎片，没有结论

- 逐轮提取记的是"这一轮发生了什么"。一晚上的讨论定了什么、改成了什么、否掉了什么，**散在十几条记忆里，没人给结论**。
- 新增 `summarizeSessionWithLlm()` + `sessionSummary` 配置（`enabled` / `rounds` 默认 12）：按会话攒轮次，够了就把**整段对话**交回模型总结成 0-3 条结论。
- 提示词刻意与逐轮提取分工：*"逐轮记的是某一轮发生了什么，你要记的是整段下来落在哪里——同一个决定被反复修改过，只写最后落定的那个"*。
- 汇总条目与逐轮条目**同规格入库**（同样带 abstract / theme / keywords，同样进图谱与注入加权），走同一个串行队列。

### 二、缺口二：纠正只会新建，于是新旧并存

- 用户说"不对，我改成 X"时，旧实现**只会新建一条**——旧的那条还留着。库内长期存在新旧并存的矛盾记忆（身高、家庭关系、择偶倾向都出现过"旧记 + 更正记"），根子就是**提取时看不见库里已经写了什么**。
- 现在提取前先检索库里已有的相关记忆（top-5），连同 id 一起交给模型，让它判断这轮是"新增"还是"在推翻旧的"：输出 `items[].supersedes`。
- 命中 `supersedes` 的条目走**纠正分支**：`store.update()` 更新那条既有记忆（**旧内容进世界线版本，可回滚**），而不是新建。
- **硬性白名单**：只接受本次真的喂给模型的那些 id——模型凭空编一个 id 也改不动无关记忆（测试钉住）。
- 提示词明确"宁可留空，不要误杀"：只有旧记忆**已经不对了**才填 supersedes，仅仅相关或补充信息的留空数组。

### 三、可测性重构

- 把"提取结果落库（含纠正分支）"抽成导出函数 `applyExtractedItems()`，让这条核心逻辑第一次能被直接单测，而不是只能靠端到端碰运气。

### 四、验证

- 新增 `test-conversation-flow.mjs`（**27 项**）：纠正走更新分支且旧内容进世界线 / 模型编的 id 改不动无关记忆 / 一轮里同时发生"纠正 + 新建" / supersedes 字段清洗（非数组、非 mem- 前缀、超量）/ 提取时已知记忆真的进了提示词 / 会话汇总把每轮都当输入且只收结论 / 空轮次拒绝 / 汇总条目是一等记忆。
- **全量 20 套 570 项全绿**；客户端 bundle 重建（新增「会话级汇总」设置区块 + 「自动执行治理」开关）；`tools/deploy.mjs` 同步并 md5 复验一致。

### 五、本机需要做的

- 重启 DSH 生效。设置面板新增两块可调项：「记忆管家 → 自动执行治理」与「会话级汇总」。

## v0.12.1 — 管家真治理：整合第一次真的发生了，而且看得见（2026-09-21）

起因：用户原话「**AI 对记忆的维护和整合我没有明显的感觉**」。查下去，这不是感觉问题，是事实问题。

### 一、为什么"没感觉"——因为真的什么也没做

- 管家自动巡检跑过 12 次，**每一次都是 `dryRun: true`**：算出候选、写一行日志、结束。不合并、不归档、不删除。
- 库里唯一自动发生的"整合"是**文本拼接**：写入期若发现关键词 Jaccard ≥ 0.8，就把新内容用 `\n---\n` 接到旧内容后面。库内实测 **101 条记忆被这样拼过，最长一条 9099 字**，同一个"早上好啊"被拼了三遍——那叫堆叠，不叫整合。
- 587 条情景快照（`任务: … 结果: …`）躺在库里，老化报告只列不处理。

### 二、归档机制：治理动作第一次变得**可回退**

- `memories` 表新增 `archived` 列（老库自动迁移）。**归档 ≠ 删除**：数据、世界线版本链、图谱边全部保留，只是退出检索与注入。
- 检索三路（FTS / 关键词 / 向量）统一过滤归档；`list()` 默认不含归档（`includeArchived` 可看）；`stats()` 把**活跃与归档分开报**（`memories` 只算活跃，新增 `archived`），不再有"库还在长"的错觉。
- 新增 `memory_archive` 工具（list / restore）——治理做错了随时捞回来。

### 三、归并取代拼接：LLM 合成一条更完整的结论

- 新增 `mergeMemoriesWithLlm()`：把一组讲同一件事的记忆读完后**合成一条**——去重、保住全部关键约束（数字/版本/路径/条件/否定条件）、**矛盾处以时间较新者为准并写清新旧关系**。复用 v0.12.0 的思考档基础设施（不设 token 上限 + 时间预算 + 断点续思）。
- `store.mergeMemories()`：目标记忆内容替换为归并结论（世界线追加版本），源记忆**归档**而非删除；关键词合并、主题与抽象层级透传。
- `groupSimilarPairs()`（并查集）：归并的最小单位是"一团讲同一件事的记忆"，不是"一对"。
- **近乎重复（≥ `autoMergeThreshold` 0.95）不再拼接**，改为择优保留信息更全的一条、其余归档；0.92~0.95 的相似区间留给 LLM 归并（直接合并会丢信息）。

### 四、管家从"只报告"升级为真治理 + 做了什么看得见

- 新增配置：`housekeeping.autoApply`（默认 **true**，即用户拍板的「进取档」）/ `autoMergeThreshold` / `archiveEpAfterDays`（默认 45 天，0 = 关闭）。
- 自动档只做安全动作（近乎重复合并 + 陈旧情景快照归档）；**LLM 归并不在后台跑**——它要花模型额度，交给用户显式开启 `memory_housekeeping {consolidate:true}`。
- 陈旧情景快照归档有保护：`strength ≥ 1.5`（被访问/加固过）的 ep 不动，只收那些**从创建起就没人检索过**的。
- **可见性三处落地**：① 工具报告新增「本次治理动作」区块（合并几组、归档几条、归档累计）；② 日志新增 `housekeeping.applied`，GUI 记忆日志面板直接看；③ 治理结论写进 meta，**24 小时内下次会话预热会带一行**——「维护」这件事终于出现在模型视野里（用户看不到注入块，但模型看得到，能顺口告诉用户）。
- 顺手修：`memory_stats` 的工具 schema 只声明了 8 个字段、而 `stats()` 返回 13 个（v0.11.1 加的 `vecDim`/`vecRows`/`embedder`/`degraded` 从未进声明，而该 schema 是 `additionalProperties: false`）；`degraded` 现保证恒为布尔。

### 五、验证

- 新增 `test-governance.mjs`（**53 项**）：归档退出检索且可恢复 / list 默认排除 / 归并是"合成"不是"堆叠"（断言结果里**不含** `\n---\n`）/ 源记忆归档而非删除 / 关键词与主题抽象透传 / 并查集成团与确定性 / dryRun 与 apply 的边界 / 陈旧 ep 判据（新 ep 与高强度 ep 受保护）/ stats 分开计数 / `memory_archive` list+restore / 归并器拒绝单条输入且走不设上限策略。
- **全量 19 套 543 项全绿**；`tools/deploy.mjs` 同步并 md5 复验一致。

### 六、本机需要做的

- 重启 DSH 生效。重启后可以这样验收：`memory_housekeeping {consolidate:true, dryRun:false}` 跑一次（会真归并并归档），再看 `memory_archive` 的列表——**能看见它动了什么，也能一键恢复**。

## v0.12.0 — 提取重做：思考档 + 时间预算 + 断点续思（2026-09-21）

起因：主人提的三件事——「提取时要有更多思考，不能潦草写出来」「记忆的维护和整合我没有明显感觉」「日常对话的提取流程要优化」。一查库，第一件事不是"质量不够好"，而是**已经瘫了 6 天**。

### 一、事故复盘：蒸馏自 2026-09-16 起 100% 失败（真故障）

- 日志对账：`write.refined` 最后一次成功是 **9-16 14:48**；此后 `write.fallback` 累计 **281 次**、`write.refined` **0 次**。也就是说 9-17 之后写进库的"记忆"，全部是降级路径的原始文本（`任务: … 结果: …`），寒暄、`(无输出)`、过程噪音一视同仁。
- 根因实测（直连端点复现，非推测）：
  - `reasoningEffort: 'off'` 在 pi-ai 适配器里被翻译成**省略 reasoning 参数**（见 `resolveReasoningLevel` 注释：*off is translated to omitting the reasoning option*），于是由上游模型默认说了算；
  - 本机 `opencode-go / deepseek-v4.1-flash` 默认**强制思考**，`maxTokens: 800` 实测 `completion_tokens=800, reasoning_tokens=800, content_len=0` —— 预算被思考吃光，正文一个字没吐出来；
  - 空字符串 → `JSON.parse('')` → `Unexpected end of JSON input` → 降级。
- **同源事故第二次**：2026-09-08 踩过一模一样的坑（当时解法是"显式传 off + 上调 maxTokens"），换模型即复发。结论：**不要再试图关掉思考去省预算**。

### 二、新模型：不设 token 上限，只用时间约束，掐断了接着写

- `refiner.maxTokens` 语义改为 **0 = 不限制**（默认 0，不传该参数）。思考想多久都行，约束只剩时间。
- 新增 `refiner.timeBudgetMs`（默认 **120000 = 2 分钟**）：到点主动 `AbortController.abort()` 掐断，**已收到的内容一律保留**，随后把"已写部分"原样带回给模型续跑（断点续思）。
- 新增 `refiner.maxContinuations`（默认 **3**）：续跑轮数上限；若某次续跑一个字都没写出来 → 判定模型不肯接话，立即停止，不空烧配额。
- `refiner.reasoningEffort` 默认由 `off` 改为 **`low`**（明确要思考；off 的语义已在配置与 GUI 里澄清为"让上游用默认"）。
- 思考流不再被丢弃：旧实现只收 `text-delta`，现在同时收 `reasoning-delta`，思考字数记入日志 `meta.reasoningChars`。
- 兼容：跑题（纯叙述、无 JSON 结构）仍走 v0.9.27 的强化指令重试一次；有 JSON 结构但没闭合/被截断才走续跑。

### 三、提取质量：先判断，再写

- 输出结构升级为 `{ analysis, items: [...] }`：
  - `analysis` 是**判断过程**（这轮发生了什么 / 值不值得记 / 写的时候必须保住哪些关键约束），不再是黑箱；
  - `items` 支持 **0-3 条**，一轮对话里的决策与偏好分开成条，不再硬压成一条。
- content 硬性质量要求：自包含（不出现"上面/这次"）、带判据（数字/条件/边界/原因）、不写过程、不写元信息（"用户说/助手说"）、一条只讲一件事。
- 明确拒绝：寒暄、确认性回复、无实质输出的轮次 → `items: []`（这是**正常路径**，不再降级成原始文本入库）。
- 规格兜底：`normalizeItems()` 白名单校验 + 最多 3 条；旧格式（顶层 `content`）仍能解析，旧调用点不破。

### 四、写入侧：不再让噪音和无界并发进来

- 价值门新增 `isTrivialTurn()`：纯寒暄/确认（"继续""好的""早上好啊""谢谢"）且助手无实质输出 → 跳过。**但短偏好必须留下**——"不要堆代码""改成 12 步""我认"这类极短表达含信号词，一律放行（库内实测这类轮次**曾被完整沉淀**，"早上好啊"还被去重合并拼了三遍）。
- 提取改**串行队列**：思考档单次可能跑满时间预算，多轮并发会互相抢流、打爆配额。队列上限 12，超限丢最旧并记日志（保留最近对话）。
- 规则路径的关键词也过 `pickKeywords()` 泛词/虚词过滤（旧实现直接 tokenize 取 40 个，把"现在/什么/这个/用户"塞进库——实测平均 22.9 个关键词、高频前 25 名里 8 个是口语虚词）。
- 日志新增 `write.no_value`（模型判断不值得记 + 理由）、`write.prefiltered`、`write.queue.dropped`；`write.refined` 带上 `analysis` 与 `meta`（几次调用 / 是否续跑 / 是否被掐断 / 思考字数）——**"AI 刚才想了什么"从此可审计**。

### 五、顺手修掉：连通性自检端点从上线起就没成功过

- `/dsh-memory/health` 每次都返回 `Cannot read properties of undefined (reading 'live')`。根因是 handler 里写 `payload.embedder`，而报告的字段叫 **`embedding`** —— 一个字段名笔误，让自检按钮点了只看到报错。
- 修法不止改名字：把日志行抽成纯函数 `healthLogDetail()`（全程可选链兜底），并补 6 项测试覆盖它。原 20 项 health 测试只测底层报告函数、没覆盖这条日志行，所以一直绿。

### 六、验证

- **新增 `test-refiner-thinking.mjs`（50 项）**：不设上限时 `maxTokens` 确实不传、正数仍透传、思考流被收、时间预算掐断后带断点续跑并拼出完整 JSON、空输出（事故形态）能被续跑救回、模型不接话立即停、续跑轮数上限、跑题仍走强化重试、多条目与旧格式兼容、`items` 为空不写库、价值门 10 种短消息判定、关键词泛词过滤。
- `test-health.mjs` 20 → **26 项**（新增字段名对齐 + 残缺报告不炸端点）。
- **全量 17 套 465 项全绿**；客户端 bundle 重建；`tools/deploy.mjs` 同步并 md5 复验一致。
- **真机端到端实测**（合成数据，直连 opencode-go）：耗时 4.0s、思考 915 字、正文 583 字，产出 `analysis` 174 字判断 + **2 条独立记忆**（决策 + 偏好），主题自动打标、关键词精准（`注入步距` / `steady` / `成果词`）。对照事故配置（off + 800）仍是正文 0 字。

### 七、工具写入不再「孤岛」（2026-09-21 二次交付）

把「AI 怎么把记忆加进去」这条链路挖到底时，发现**另一个哑故障**：

- 实测口径：库内可追溯的 **41 条 `tool.add` 记忆，100% 无图节点、无任何连边**；同期的自动提取路径有 **57%** 连上了边。
- 根因：`lib/tools/memory.js` 的 `memory_add` 只调了 `store.add`，**没走图谱那只手**（`graphLink` / `linkBefore` / `linkSemantic` 一个都没调）。而自动路径的 `upsertMemory` 三个都调。
- 顺带修掉同一条路径上的两个小缺陷：
  - **关键词用 `tokenize` 的 2-gram 碎片兜底**：`提取链路根治` 会切出 `取链`/`路根` 这种跨词边界的碎片。`pickKeywords` 新增**虚词边界过滤**（首尾是「的/了/是/在/里/等/于…」的 2-gram 一律丢弃），实测挡掉 `器里`/`里等`，同时保留 `链路`/`根因` 这类真词。
  - **工具声明里没有 `abstract` / `theme`**：主动加的记忆永远没有抽象层级 → 注入加权（principle ×1.5 / event ×0.7）对它完全不生效（库内 392 条 sm 无抽象标记，含这批）。
- 现在 `memory_add` 与自动提取**同一口径**：同一套关键词过滤、同一组连边、同一套维度；`features.graph=false` 或 `layer=ep` 时不碰图谱。
- 新增 `test-tool-add.mjs`（20 项）：连边必调 / 关图不调 / ep 不调 / abstract+theme 透传 / 碎片过滤 / content 归一化 / 写后可检索。

### 八、顺手修掉：自检探针自己也犯了同一个病

重启后实测发现 `/dsh-memory/health?llm=1` 的提取项报红，但提取链路本体是好的——**探针自己硬编码了 `maxTokens: 32`**，
思考型模型光思考就要几百到上千 token，32 的预算下正文必然为空 → 自检永远误报「提取模型返回空文本」。
现改为与真实提取路径一致：不设 token 上限 + 时间预算（探针取 25 秒，且不得比用户给真实提取的预算更宽松），
掐断但已有正文仍判可用。`test-health.mjs` 26 → **31 项**（新增 5 项钉住这条）。

### 九、本机需要做的

- `settings.yaml` 的 `memory.refiner` 建议清掉历史遗留的 `maxTokens: 800`（不填即为 0 = 不限制）；`reasoningEffort` 不填默认 `low`。
- **重启 DSH 生效**（`lib/` 改动）；客户端面板新字段刷新页面即可。
- 存量 41 条孤岛记忆可在后续「维护批次」里做一次补连（v0.11.0 的 `tools/semantic-links.mjs` 即为此用途）。

## v0.11.2 — 注入节奏改档位制：激进 / 平稳 / 懒惰（2026-09-18）

起因：主人问「现在是多少步检索注入一次？有些太过频繁了，我想限制在 12 步」——一查，生效值是 **2 步**（`cordis.patch.yml` 组合层写的 `stepInterval: 2`，默认值 10 被它盖掉），所以几乎每两步就全量重检索一次；而他想填的 12 **根本填不进去**——schema 上限是 10，`Config({stepInterval:12})` 直接抛 `expected number <= 10 but got 12`。于是干脆把这件事做成档位。

### 一、档位制（用户提案）
- 新增 `injectPace`：`aggressive` 激进=**4 步** / `steady` 平稳=**12 步**（默认）/ `lazy` 懒惰=**30 步** / `custom` 自定义。
- 单一来源 `lib/config.js` 的 `resolveStepInterval(cfg)`：`inject.js` 一律经它取生效步距，别处不再自己算；`custom` 档才读 `stepInterval`（上限由 10 放宽到 60）。
- 边界刻意偏保守：非法档位（写错单词）→ 回落「平稳」；`custom` 的非数/小于 1 → 回落「平稳」而**不是**夹成 1（夹成 1 等于"每步都检"，与"想少打扰"的意图正好相反）；超上限 → 夹到 60。
- 档位用 `z.string()` 而非 `z.union`：宁可静默回落到平稳，也不让 settings 校验抛错把整个 memory 命名空间带下水（本插件第一原则：dsh 必须存活）。
- 注入日志新增 `interval` + `pace` 两个字段——以后对账"到底是哪档在跑"不用猜。

### 二、GUI
- 「检索与注入」区新增**注入节奏**下拉（三档 + 自定义，每档附一句人话说明），并把「当前生效：每 N 步重新检索一次」直接写在提示里。
- `stepInterval` 数字输入框在非「自定义」档下明确提示"暂不生效"，避免"改了数字没反应"的困惑。
- 面板 `TEXT_TOP` 放行 `injectPace`（否则顶层字符串会被数值校验拦下，保存按钮点不动）。

### 三、验证
- `test-inject-pipeline.mjs` 31 → **55 项**：档位解析（4/12/30/自定义/缺失/写错/越界）、平稳档 121 步节奏（注入于 1,13,…,121）、懒惰档 61 步节奏（1,31,61）、自定义档 12 步节奏、schema 边界（12 合法 / 61、0 非法 / 默认 steady）、**客户端档位表与 `INJECT_PACE_STEPS`/`LABELS` 同源校验**（防"UI 写 12、代码跑 10"漂移）。
- 全量套件 17 套全绿；客户端 bundle 重建并同步部署副本。

### 四、本机配置
- `settings.yaml` 的 `memory` 段：`injectPace: steady`（每 12 步，正是主人要的）+ `stepInterval: 12`（自定义档起点）。
- `cordis.patch.yml` 的 `stepInterval: 2 → 12`：2 是历史遗留值，现在只在 custom 档生效。

### 五、实测留痕（"改了到底生效没"不靠嘴说）
- **重启前**：注入日志的步号间隔仍是 2（205→208→210→212→214），且日志里没有新增的 `interval`/`pace` 字段 → 跑的还是启动时那份代码与 base 配置（内存里的旧 schema 上限 10，会把用户层的 12 判非法 → 回落启动时的 base=2）。
- **重启后**：本地按新 schema 复算组合层 → `injectPace=steady`、`stepInterval=12`、生效步距 **12**（命令行实测输出）。重启后日志每条 inject 都会带 `interval:12 / pace:"steady"`，可直接对账。

## v0.11.1 — 配置可信度三修：部署副本漂移（真故障）/ 连通性自检 / 设置面板回归原生外观（2026-09-17）

起因：主人问「这插件到底能不能在 DSH 里自由自定义配置」＋「dsh 的设置界面和记忆插件的不一样，我们多做了一个背景」。查出三件事，其中第一件**已经在造成真实损害**。

### 一、部署副本漂移 → 向量路实际已瘫（真故障，非隐患）
- md5 对账发现部署副本落后 4 个文件，其中 `lib/index.js` / `lib/embedder.js` 停在 9/8 的版本，**缺 v0.9.31 的降级保护**（`degraded` 禁破坏性维度迁移 + `reembedMissing` 批次重试 + 8s→30s 超时）。
- 后果链：远端嵌入 402 → 落到 rule 兜底 → 旧版没有降级护栏 → 向量表被迁移成 **256 维**（1021 条全是 rule 哈希向量），语义向量路事实上停摆，而日志里只有一行 warning。
- 交付 `tools/deploy.mjs`：`node tools/deploy.mjs` 同步并复验 md5，`--check` 只比对（有漂移退出码 1）。README 增「开发期部署」小节记录这次事故。

### 二、「● 已配置」≠「真的能用」→ 连通性自检
- 硅基流动账户余额不足（HTTP 402 `Sorry, your account balance is insufficient`）时，设置面板仍显示「● 已配置（MEMORY_EMBEDDING_API_KEY）」，`memory_stats` 也只回一个 `vector: true`。
- 新增 `GET /dsh-memory/health`（`?llm=1` 附带提取模型探针）：对嵌入 / 重排 / 提取各发一次**真实最小请求**，同时报配置值、凭据存在性、向量路实况（谁在算向量、几维、多少条、是否降级）。面板顶部新增「连通性自检」按钮。
- `MemoryStore.stats()` 补 `vecDim / vecRows / embedder / degraded`——`embedder: "rule"` 就是那句一直没人说出口的实话。
- 新增 `test-health.mjs`（20 项）：402 原文透传、降级态显形、未启用不假检、探针自身绝不抛。

### 三、设置面板回归宿主视觉（去掉"多出来的背景"）
- 面板此前用 `#1a1a1a` 卡片 + `#444` 边框自画分区，在浅色主题下像贴了几块黑砖，与原生设置页（标题 + 分隔线 + 原生控件）对不上。
- 全量改走宿主设计令牌 `--dsw-alias-*`（14 种：label / border / bg-layer / state-warn…），分区改为「标题 + `border-l2` 分隔线」，输入框对齐原生度量（高 32 / 圆角 8 / `bg-layer-1`）。
- 顺带纠一处错误说明：面板曾写「嵌入与重排改动 live 生效」——实际上 `embedding.*` / `reranker.*` 都在**初始化期**读取，必须重启；README 新增「哪些改动 live / 哪些需重启」对照表，需重启字段逐项标注。
- 未动 `graph.jsx` / `logs.jsx`（仍是深色浮层自绘）——那是面板不是设置页，如需一并对齐令牌可另起一轮。

### 四、顺带查清「提取模型为什么一直在降级」
- 查日志发现 `write.fallback {"err":"Unexpected end of JSON input"}` 从 12:28 起**连续 5 次**（含配置切换后的 13:03/13:04），而下一次成功的 `write.refined` 还停在前一天 14:48——**提取路 100% 静默降级，界面上完全看不出来**。
- 根因一：旧路由 `deepseek-official / deepseek-v4-flash` 的密钥**余额不足**（实测 HTTP 402 `Insufficient Balance`）→ 空回包 → `JSON.parse('')` 报 "Unexpected end of JSON input"。本次把 `refiner.provider/model` 换成 `opencode-go / deepseek-v4.1-flash` 正好绕开这条死路。
- 根因二（可观测性）：`write.fallback` 日志**不带 route**，失败无法归因。已补 `route: "<provider>/<model>"` 到 `write.fallback` 与 `write.refined`。
- 新路由实测可用（直连 `/chat/completions` 复刻 refiner 调用：严格 JSON 可解析，reasoning 关/开都正常），重启后由「连通性自检」第三行复核即可。

### 验证
- 全量套件：`test.mjs` 52 / phase2 18 / phase3 21 / housekeeping 38 / events 21 / incremental 21 / update-append 9 / edge-types 15 / keyword-filter 8 / link-suggest 18 / semantic-links 11 / graph-geometry 66 / inject-pipeline 31 / crash-safety 10 / profile 26 / health 20 全绿；`test-record` 6 项通过（4 提醒）。
- `test-embedder.mjs` 24 项中唯一失败是**真实 API 402**（余额不足）；该测试已改为「验当前配置的模型」并输出明确结论，不再硬编码老模型静默跳过。
- 客户端 bundle 132492 → 139073 B，冒烟（桩 React 加载 + apply 注册三处插槽）通过；`tools/deploy.mjs --check` 全等。

### 待主人决策
- 硅基流动余额：充值后重启即按新配置（`Qwen/Qwen3-Embedding-8B` + `Qwen/Qwen3-Reranker-8B`，各 4096 维）自动全量重嵌入（1021 条）；或改用百炼端点（实测 `qwen3.7-text-embedding` 1024 维可用；该端点 rerank 404，重排需另想办法）。
