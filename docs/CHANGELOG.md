...
## v0.10.9 — 主题区域语义修正：选中/聚焦 = 包住整主题全部成员（2026-09-13）

用户反馈：「并不能让所有的节点都被圆圈包裹住」。上一版为了"不糊"，把区域限制在**密集团**（densityCore 滤掉稀疏末端），于是选中某主题时，那些空间上散落的同主题节点落在圈外——在**筛选场景下这是错的**：此刻只高亮一个主题、其余已变灰，本就不需要"只圈密集团"，散落成员漏在圈外反而费解。

### 改动
- **两种语义分开**（`graph.jsx`）：
  - **选中/聚焦某主题**（主题筛选下拉 或 focus 模式下悬停/选中节点）→ 画**整主题**：形状覆盖该主题**全部成员**（散落的也包住），标签标主题总数。
  - **常显（always）**→ 仍只圈各自**密集团**（多主题同屏时若不这样会糊成一片，这条不变）。
- **覆盖自检**：钉住顶点的多边形若因节点漂移漏掉成员 → 立即作废该计划、下一帧重建（重建会把漏点重新纳入顶点集），保证"全包住"是硬约束而非近似。

### 验证
- 覆盖量化（真实快照、主题「用户/荣格」45 成员、400 帧连续测）：
  - 凸包 + 钉顶点 + 时间平滑：**有漏点的帧 0/400，单帧最多漏 0 个**
  - 圆形（每帧按全部成员重算）：**单帧最多漏 0 个**
- 几何/缓存测试 66 项全绿；bundle 128821 B；版本 0.10.8 → 0.10.9；副本 `lib/client.js` + package.json md5 同步。
- ⚠️ 前端刷新生效）：用工具栏「主题筛选」选一个主题 → 该主题**全部**记忆都会被形状包住，其余变灰。

## v0.10.10 — 布局是真的平衡了：预热两段式 + 破同心环 + 拖动局部化（2026-09-13）

用户实拍："节点都变成按同样顺序排列了，这就是你说的全局平衡？完全不对；拖动还会导致全局扰动。"

### 根因：把"alpha 衰减杀死的环形初位"当成了平衡
- 预热判据是"速度小 → 收敛"，但速度小是因为 **alpha 衰减到 0 把力杀死了**，布局远未平衡。缓存下来的其实是"环形初始布局 + 一点扰动"：有连接的核心会散开，而**零连接记忆不受力耦合，永远停在初始那条同心弧上** → 满屏"按顺序排好的圈"。又因为落盘后每次打开都复用，这种假平衡被永久固化。

### 修复（`client/graph.jsx`）
1. **预热两段式**：物理上"平衡位置与 alpha 无关"（所有力同倍缩放，alpha 只决定步长）→ ① 固定 alpha 0.45 快铺 400 步；② alpha ×0.985/步 慢衰减 400 步收尾，落到真实力平衡。实测 800 步 / 约 250ms（一次收敛后落盘，后续打开直接复用）。
2. **破同心环**：① 每节点回中力按 id 确定性微差（0.65~1.35）→ 平衡半径不再整齐划一；② 零连接记忆额外吸向**本主题已连接成员的质心**（有主题的才生效）→ 同主题记忆真的聚到一起；③ **无连接节点之间斥力 ×0.35**（它们无结构关系，允许靠紧）→ "噪声光环"半径均值 887 → 718（收紧约 19%）。
3. **拖动局部化**：拖动时只让**图距离 ≤2 的邻域**参与（BFS 作用域），范围外节点速度与位移冻结 → 不再"拖一个、全世界跟着动"。
4. **缓存版本化**：键升级为 `dsh-memory:graph-layout:v2`——旧缓存是环形假平衡，必须自然作废，否则刷新后仍看到坏布局。

### 验证
- 离线渲染复核（真实快照）：孤立节点半径离散度 std/mean 0.30、直方图峰值占比 26~29%（不再集中于单一环带）；出图确认核心有结构、外围为弥散云而非同心弧。
- 66 项几何/缓存测试 + `test.mjs` 52 项全绿；bundle 130525 B；版本 0.10.9 → 0.10.10；副本 `lib/client.js` + package.json md5 同步。

## v0.10.11 — 节点不再重合：碰撞规避硬约束 + 半径改为布局常量（2026-09-13）

用户实拍：放大后可见**大量节点重合**，要求"各自节点单独的引力/斥力平衡，并且不会重合"。

### 根因
- v0.10.10 为收紧"噪声光环"把**无连接节点之间斥力降到 0.35**，又给零连接记忆加了"吸向主题质心"的点吸引子 → 多节点被拉向同一点，互相叠住。
- 更根本的：节点半径是 `(4+deg×0.55)/√k`（**随缩放变化**）→ "不重合"在数学上就无法作为布局约束，放大后必然重叠。

### 修复
1. **碰撞规避（位置式硬约束）**：每步在力更新后跑一遍网格加速的分离（O(n)），任何一对节点间距 < `r_i + r_j + 6` 就沿连线推开（各推一半、留 20% 欠松弛防抖）。力负责"自然布局"，碰撞负责"物理上不重叠"。
2. **节点半径改为布局空间常量** `r = 5 + min(deg,14)×0.7`（不再 /√k）：半径不再随缩放漂移 → 任何缩放倍率下都不重合，观感也不再忽大忽小。命中判定、区域 padding 同步改用该常量。
3. **无连接节点斥力回到 0.7**（0.35 太弱，是重合的直接推手；有了硬约束不需要压这么狠）。
4. **缓存键升 v3**：旧缓存布局可能带重合，自然作废。

### 验证
- 离线复核（真实快照 490 节点、800 步收敛）：**重合对 0**（修复前有明显重叠；欠松弛 0.6 时为 1 对、最紧 0.90×所需间距 → 改为 0.8 后归零）。
- 孤立节点半径离散度 std/mean 0.32、直方图峰值占比 26%（既不成同心环、也不塌成一坨）。
- 66 项几何/缓存测试 + `test.mjs` 52 项全绿；bundle 132413 B；版本 0.10.10 → 0.10.11；副本 `lib/client.js` + package.json md5 同步。

## v0.10.12 — 斥力模型重写：每一对节点都是独立斥力源（全对 1/d²，无截断）（2026-09-13）

用户实拍并指出根子："即使没有强烈重叠，也有过于靠近……你没有把每一个点都作为一个独立的引力斥力源来编写代码。"

### 根因：旧斥力是"截断 + 封顶"的伪全对力
- `if (d >= 2.2*k) continue`：**超过 ~114px 就完全没有斥力** → 远处不受力，图自然裂成"一团一团贴着的小块 + 大片空白"。
- `min(k²/d, 0.6k)`：近距离被 cap → 挤在一起时推不开。
- 于是"不重合"只能靠 v0.10.11 的**碰撞推挤**兜底 —— 那本质是把节点当弹珠挤到**刚好接触**，正是"过于靠近"的直接来源。

### 重写
- **全对 1/d² 斥力、不做距离截断**：`f = repulsion × REP_C × k² / max(d,12)²`（REP_C = 8）。近处强推、远处留微力，间距由力平衡自然长出。
- 节点半径 `7 + min(度,14)×0.9`（间距变大后补偿屏上尺寸）；碰撞规避保留为**极小概率兜底**（不再承担间距职责）。
- 主题锚点力 0.012 → 0.008（点吸引子曾是"堆叠"推手之一）。
- 缓存键升 v4（旧力模型产出的"团状贴紧"布局作废）。

### 验证（真实快照 490 节点、同款两段式预热）
| 力模型 | 近邻间距 p5 | 中位 | p95 | 重合对 |
|---|---|---|---|---|
| 旧（截断+封顶） | **16**（≈贴着，节点半径最大 14.8） | 73 | 114 | 0 |
| **新（全对 1/d²，C=8）** | **60** | 108 | 143 | 0 |

- 最小间距提升 **3.75×**，且不再有"贴死"的节点对；图幅 2456×2291 → 2936×3105（fit 缩放仍能全景容纳）。
- 66 项几何/缓存测试 + `test.mjs` 52 项全绿；bundle 132309 B；版本 0.10.11 → 0.10.12；副本 `lib/client.js` + package.json md5 同步。

## v0.10.13 — 布局收敛门：先算到"全部静止"再展示 + 质量档位（2026-09-13）

用户诉求：① 点进去时先把布局**算到所有点都不动**再展现；② 大图/弱机要有降级手段。

### 实现
1. **收敛门 + 分块计算（不卡 UI）**：进入图谱先跑布局，**每帧最多算 12ms**（`requestAnimationFrame` 分块），跑到"最大节点速度 < 0.02"（真静止）才揭幕；期间显示**进度覆盖层**（百分比 + 残余速度 + 降档提示），算完立刻落盘 → 下次打开命中缓存**零等待**直接展示。
2. **缓存命中即时出图**：坐标已是收敛态 → 只做 24 步微松弛，不展示等待（这是"每次打开都要重算"与"打开即平衡"的平衡点）。
3. **质量档位**（新配置 `graphView.layoutQuality`，设置面板「记忆图谱」可选）：
   - `precise`（默认）：算到静止才展示，预算 4s / 8000 步
   - `balanced`：限时 1.2s / 3000 步
   - `instant`：不重算，直接用缓存或初始布局
4. **自动降档**：节点数 >900 时 precise → balanced；>1800 且已有缓存 → instant（无缓存则 balanced）——大图上 all-pairs 步进是 O(n²)，降档避免开面板长期等待；覆盖层会说明降档原因。

### 结构
- 新增纯函数模块 `client/layout-policy.js`（`resolveLayoutPlan` / `progressLabel`）——策略可测，渲染侧只负责分块执行。

### 验证
- 守护测试 **76 项**（新增 layout-policy 10 项：三档预算/步数、非法值回落、900/1800 两档自动降档、有缓存走 instant、速度阈值、进度文案）；`test.mjs` 52 项、`test-crash-safety` 10 项、`test-inject-pipeline` 31 项全绿。
- bundle 138115 B；版本 0.10.12 → 0.10.13；副本 `lib/client.js` + `lib/config.js` + package.json md5 同步。
- ⚠️ 前端刷新生效。首次进入会看到进度条（算到静止），之后每次打开都是瞬时的收敛布局。

## v0.10.14 — 拖动实时性修复 + 斥力改 O(n)（网格平滑截断 + 远场近似）（2026-09-13）

用户报：拖动节点没有实时动效，过一会儿才跳到拖动位置；并追问"能否在同等效果下把负载降到 O(n)"。

### 一、拖拽两处缺陷（都是本系列引入的）
1. **拖拽节点不"权威"**：每帧的力与碰撞推挤会把鼠标正抓着的节点推开（前方有冻结节点堆时尤其明显）→ 看起来跟不上手。修复：新增 `dragTarget`（世界坐标），每步末尾把拖拽节点**强制归位**到鼠标目标，受力与碰撞都不得移动它。
2. **rAF 循环会被单帧异常杀死**：一旦某帧抛错，整条循环链断裂 → 画布静默冻结，节点位置要等下次偶发重绘才"跳"过去（正是用户描述的延迟）。修复：循环体 `try/catch` 包裹，异常只记一次日志、循环照常续跑；拖拽时 `onMove` 也**直接重绘**，不再依赖循环存活。

### 二、斥力改 O(n)：均匀网格 + 平滑截断 + 远场近似
- **本地**：均匀网格（格 = R = 3k）查询 3×3 邻域 → 每节点只与常数个邻居互作用；力乘 `(1-d/R)²` **平滑衰减到 0**（力与导数在边界都连续）。这正是旧版做错的地方——它截断是**硬**的（力的悬崖），才裂出"团块 + 空白"。
- **远场**：截断丢掉的长程斥力用**一级 Barnes-Hut**补回——把 3×3 邻域之外的节点汇总为位于全局质心的一个质体，距离取 |r−质心|（下限 R），每节点 O(1)。
- 标定：`REP_C=8 / 补偿 ×2 / 远场 ×1.2`。

### 三、实测（合成图，同点集同边集公平对比）
| 节点数 | 全对 O(n²) ms/步 | 网格 O(n) ms/步 | 加速 |
|---|---|---|---|
| 500 | 0.90 | 0.67 | 1.4× |
| 2000 | 8.48 | 2.40 | 3.5× |
| 4000 | 37.8 | 7.6 | 5.0× |
| **8000** | **293.5** | **15.3** | **19.2×** |

视觉等价（同起点各跑 400 步）：近邻 p5 **52 : 59**、中位 **68 : 80**、图幅半径 **1462 : 1431**（网格 : 全对）——间距量级一致。
- 因斥力已是 O(n)，**自动降档阈值放宽**：>2000 才回落均衡、>6000 且有缓存才走极速（原 900/1800 是为 O(n²) 设的保守值）。

### 验证
- 76 项几何/缓存/策略测试（策略阈值同步更新）+ `test.mjs` 52 项 + `test-crash-safety` 10 项全绿；bundle 140356 B；版本 0.10.13 → 0.10.14；副本 `lib/client.js` + package.json md5 同步。

## v0.10.15 — 回退：图谱实时律动恢复（撤销 v0.10.13/0.10.14 的渲染路径改动）（2026-09-13）

用户反馈「现在不能实时渲染了」→ **回退到 v0.10.12 的图谱渲染路径**（`client/graph.jsx` / `client/settings.jsx` / `lib/config.js` 精确还原到 `2d4a0ba`），实时物理与拖拽恢复正常。

### 回退范围
- 撤销 v0.10.13 的「布局收敛门 + 进度覆盖层 + `layoutQuality` 档位」与 v0.10.14 的「O(n) 网格斥力 + 远场近似 + 拖拽权威归位 + 循环 try/catch」。
- 删除实验性文件 `client/layout-policy.js`（及其守护测试节）；`client/layout-cache.js` 一并还原。
- 保留 v0.10.12 的行为：**实时力导向（alpha 地板 0.02 持续律动）+ 全对 1/d² 斥力 + 碰撞兜底 + 布局落盘缓存（键 v4）**。

### 复盘（为什么要记一笔）
- 这两次改动**无法在本机实测浏览器渲染路径**（插件 client 只能在 DSH GUI 里跑），却按"测试全绿"就发布了——而它们的风险恰恰在运行时（rAF 循环、状态机、交互）。
- v0.10.14 加的循环 `try/catch` 兜底把运行时异常**吞掉只记一次日志**，于是故障表现为"画面不动"而非报错，进一步掩盖了根因。
- 教训：**渲染路径的改动必须在真实 GUI 里看过才发布**；"测试全绿"只覆盖纯函数，不覆盖画布循环与交互。

### 后续（待主人点头再做）
- 收敛门（"算到静止再展示"）与 O(n) 斥力（网格 + 平滑截断 + 一级 Barnes-Hut 远场，实测 8000 节点 19×）两项的**实现与基准数据仍有效**，但需按"先在 GUI 实测、再发布"的节奏重做。

### 验证
- 66 + 52 + 10 + 31 项测试全绿；bundle 132309 B（与 v0.10.12 一致）；版本 0.10.14 → 0.10.15；副本 `lib/client.js` + `lib/config.js` + package.json md5 同步。

## v0.11.0-dryrun — 语义连边：纯算法判定 + 只读 dryRun 报告（2026-09-13）

用户构想：记忆载入图谱前先判语义，按相似度与已有记忆连线，且**最好不需要 AI 介入判断**。→ 结论：**可行**，因为向量在写入管线里本就已算好（检索必需），判定只需确定性规则。**本条目只交付判定逻辑与只读报告工具，尚未写入任何数据。**

### 交付物（新增，均未被运行时引用 → 零影响）
- `lib/link-suggest.js`：纯函数四道闸门——① cos ≥ τ（默认 0.78）② **互为最近邻 MNN**（实测砍掉约一半单向噪声，是"真实有效"的主力判据）③ 跨 scope 更高阈值（默认 0.85，治"不同项目但都讲工作流"的假阳性）④ 每条记忆最多 2 条新边；两端都须在参与层内（默认 sm↔sm）；同一对只输出一次。
- `tools/link-dryrun.mjs`：只读报告工具（`node tools/link-dryrun.mjs [--tau 0.80] [--tau-cross 0.88] [--with-ep] [--samples N]`）——读 4096 维真实向量算 KNN，输出新增边数、跳过原因、度数分布变化，并抽样"最相似/最不相似/跨 scope"三组供人工判读。
- `test-link-suggest.mjs`：18 项守护测试（四道闸门、去重、上限、层级过滤、MNN 开关、确定性）。

### 生产库实测（786 条记忆 / 234 条现有活跃边，全对 KNN 2.7s）
| 口径 | 新增边 | 涉及记忆 | 跨 scope | "双方原本都孤立、被接回主图" |
|---|---|---|---|---|
| **仅 sm↔sm（推荐，τ=0.80/0.88）** | **160** | 173 | 6 | **76** |
| 允许 sm↔ep（τ=0.80/0.88） | 284 | 276 | 9 | 183 |
| 仅 sm↔sm（τ=0.78/0.85，较松） | 251 | 247 | 30 | 121 |

0 度节点从 594 降到 432（τ=0.78 口径）／更严口径下 595→519。

### 质量判读（抽样）
- 高相似端（0.90+）：同一里程碑写了两遍的近重复对（如 "v0.4.3 完成" ↔ "v0.4.3 质量改进与世界线回滚" 0.912）✅
- 中低端（0.80 附近）：同一主题的不同细节/前后修订（"蓝牙断续问题最终解决" ↔ "蓝牙断续现象与设备" 0.800）✅ 可接受
- 跨 scope（0.88+）：同域不同项目（EAC/记忆插件的两个 bug）✅ 可接受
- **反向对照**：现有 234 条边的相似度中位仅 0.721、只有 60% ≥0.7 —— 说明新边集的语义依据**强于**现有边，属"改善"而非"污染"。

### 尚未做（等用户拍板）
1. 把 160 条边写入 `memory_links`（可回滚：只置 valid_to）
2. 接进写入路径（新记忆落库即连边）
3. ep 层默认不出图（279 条无连线的会话记忆占位问题）

## v0.11.0 — 语义连边落地：存量补连 + 写入期自动连边（2026-09-13）

用户构想落地：记忆入图前先判语义、按相似度连线，且**不需要 AI 判断**——向量本就在写入管线里算好，判定全部由确定性规则完成。

### 一、存量补连（已执行，可回滚）
- 用 `tools/semantic-links.mjs --apply` 落库 **160 条 similarTo 边**（τ=0.80 / 跨 scope 0.88 / 每条上限 2 / 仅 sm↔sm）。
- 落库前 `VACUUM INTO` 做了一致性快照备份；整批边共用一个 `valid_from` stamp，回滚一条命令：
  `node tools/semantic-links.mjs --rollback 1789297374020`
- **效果核对（对比备份精确统计）**：0 度（孤立）记忆 **599 → 487**；其中 sm 层孤立 **320 → 208**（**112 条记忆被接回主图**）；ep 层 279 条未参与；**复活旧边 0 条**。
- 现有活跃边：before 113 · mentions 54 · similarTo 81 → **241**。

### 二、写入期自动连边（本次新增，需重启 EAC 生效）
- `store.linkSemantic(memoryId)`：写入后立即用 sqlite-vec KNN 取候选 → 精确余弦复核 → 逐候选验"互为最近邻" → 交 `lib/link-suggest.js` 四道闸门 → `linkMemories` 落边。**全程不调 LLM**。
- 接入 `lib/pipelines/write.js`：`graphLink` → `linkBefore` → **`linkSemantic`**，异常一律不影响写入主流程。
- 修掉两个实现期 bug（集成测试抓到）：① 候选的 KNN 名次表只映射了候选集，导致"互为最近邻"永远判不出来；② 模块默认跨 scope 阈值 0.85 与工具/落库口径 0.88 不一致 → 统一为 0.88。

### 三、测试
- 新增 `test-semantic-links.mjs` **11 项**集成测试（临时库 + 手工构造向量精确控制余弦：高相似连边、低于阈值不连、单向不连、跨 scope 守卫、度数上限、ep 不参与、向量缺失不抛；**每段用独立临时库**——共用库会互相污染，这个坑当场踩到并修掉）。
- `test-link-suggest.mjs` 18 项；全量回归 52/10/31/26/66/38 全绿。

### 四、尚未做
- ep 层默认不出图（279 条无连线的会话记忆占位）——前端改动，待主人刷新确认后再做。
